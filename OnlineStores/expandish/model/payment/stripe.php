...
class ModelPaymentStripe extends Model {		
	...
	public function getMethod($address, $total = 0) {
		...
		//var_dump( $settings['customer_groups']);
		$customer_group_id = (version_compare(VERSION, '2.0', '<')) ? (int)$this->customer->getCustomerGroupId() : (int)$this->customer->getGroupId();

        $language = (isset($this->session->data['language_id'])) ? $this->session->data['language_id'] : $this->config->get('config_language_id');

		if (!$settings['status'] ||
			($settings['min_total'] && (float)$settings['min_total'] > $total) ||
			($settings['max_total'] && (float)$settings['max_total'] < $total) ||
            ($settings['stores'] && !array_intersect(array($this->config->get('config_store_id')), explode(';', $settings['stores']))) ||
            ($settings['geo_zones'] && !array_intersect($current_geozones, explode(';', $settings['geo_zones']))) ||
            ($settings['customer_groups'] && !array_intersect(array($customer_group_id), explode(';', $settings['customer_groups']))) ||
			...
			return array(
				...
				'terms'			=> '',
				'title'			=> html_entity_decode($settings['title'][$language], ENT_QUOTES, 'UTF-8'),
			);
		}
	}
	...
	public function createOrder($order_data) {
		...
		$default_order_data = array(
			...
			'value'						=> $this->currency->getValue($currency_code),
