...
<?php
class ModelPaymentPaytabsV2 extends Model {

    public function getMethod($address, $total) {
        $this->language->load_json('payment/paytabs');

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('paytabsV2_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if ($this->config->get('paytabsV2_total') > 0 && $this->config->get('paytabsV2_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('paytabsV2_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
            ...
            $title = $this->getFieldName('paytabsV2');
