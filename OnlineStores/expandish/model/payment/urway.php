...
class ModelPaymentURWAY extends Model {
	...
	public function getMethod($address, $total) {
        ...
        $this->language->load_json('payment/urway');

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_urway_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if ($this->config->get('payment_urway_total') > 0 && $this->config->get('payment_urway_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('payment_urway_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			...
			$title = $this->config->get("payment_urway_field_name_{$language_id}");

			$method_data = array(
				'code' => 'urway',
				'title' => $title ? $title : $this->language->get('text_title'),
				'terms' => '',
				'sort_order' => $this->config->get('payment_urway_sort_order')
			);
		}
	...
	public function getCards($customer_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "URWAY_card WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function addCard($order_id, $card_data) {
		$this->db->query("INSERT into " . DB_PREFIX . "URWAY_card SET customer_id = '" . $this->db->escape($card_data['customer_id']) . "', order_id = '" . $this->db->escape($order_id) . "', digits = '" . $this->db->escape($card_data['Last4Digits']) . "', expiry = '" . $this->db->escape($card_data['ExpiryDate']) . "', type = '" . $this->db->escape($card_data['CardType']) . "', token = '" . $this->db->escape($card_data['Token']) . "'");
	}

	public function deleteCard($token) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "URWAY_card WHERE customer_id = '" . $this->customer->isLogged() . "' AND `token` = '" . $this->db->escape($token) . "'");
	...
	public function addOrder($order_info, $order_code) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "URWAY_order` SET `order_id` = '" . (int)$order_info['order_id'] . "', `order_code` = '" . $this->db->escape($order_code) . "', `date_added` = now(), `date_modified` = now(), `currency_code` = '" . $this->db->escape($order_info['currency_code']) . "', `total` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false) . "'");
	...
	public function getOrder($order_id) {
		...
		if ($qry->num_rows) {
			$order = $qry->row;
			$order['transactions'] = $this->getTransactions($order['URWAY_order_id']);
	...
	public function addTransaction($URWAY_order_id, $type, $order_info) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "URWAY_order_transaction` SET `URWAY_order_id` = '" . (int)$URWAY_order_id . "', `date_added` = now(), `type` = '" . $this->db->escape($type) . "', `amount` = '" . $this->currency->format($order_info['total'], $order_info['currency_code'], false, false) . "'");
	}

	public function getTransactions($URWAY_order_id) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "URWAY_order_transaction` WHERE `URWAY_order_id` = '" . (int)$URWAY_order_id . "'");
	...
	public function recurringPayment($item, $order_id_rand, $token) {
		...
		$response_data = $this->model_payment_urway->sendCurl('orders', $order);

		$this->model_payment_urway->logger($response_data);
	...
	public function cronPayment() {
		...
		$log = new Log('URWAY_recurring_orders.log');
	...
	public function getURWAYOrder($URWAY_order_id) {
		$qry = $this->db->query("SELECT * FROM " . DB_PREFIX . "URWAY_order WHERE order_code = " . (int)$URWAY_order_id);
		return $qry->row;
	}

	public function updateCronJobRunTime() {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `code` = 'payment_urway' AND `key` = 'payment_urway_last_cron_job_run'");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` (`store_id`, `code`, `key`, `value`, `serialized`) VALUES (0, 'payment_urway', 'payment_urway_last_cron_job_run', NOW(), 0)");
	}
	...
	public function logger($data) {
		if ($this->config->get('payment_urway_debug')) {
			$log = new Log('URWAY_debug.log');
