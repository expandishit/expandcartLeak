<?php
	...
	class ModelRewardPointsShoppingCartRule extends Model
	{
		CONST ENABLED                         = 1;
		CONST DISABLED                        = 0;
		CONST SPEND_Y_TO_REDEEM_X_POINT       = 2;
		CONST FIXED_REWARDED_POINT_WHOLE_CART = 1;
				...
				if ($this->validateCondition($condition['conditions'])) {
					switch ($rule['actions']) {
						case self::FIXED_REWARDED_POINT_WHOLE_CART:
                            ...
                            {
                                $shopping_cart_point[$rule['rule_id']] += ($product['quantity'] * $rule['reward_point']);
                            }
							break;
