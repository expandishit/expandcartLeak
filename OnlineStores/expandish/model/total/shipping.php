...
class ModelTotalShipping extends Model {
	public function getTotal(&$total_data, &$total, &$taxes) {
		if ($this->cart->hasShipping() && isset($this->session->data['shipping_method'])) {
			...
			//Apply salasa on a all order not as shipping method
			$salasa_settings = $this->config->get('salasa');
			if(is_array($salasa_settings) && $salasa_settings['status'] == 1 && $salasa_settings['is_shipping'] != 1){
				$this->language->load_json('shipping/salasa');

				$this->load->model('shipping/salasa');

				$address = $this->session->data['shipping_address'];
		        if ($salasa_settings['geo_zone_id'] != 0 && $this->model_shipping_salasa->getGeoZone($salasa_settings['geo_zone_id'], $address) == true) {
		            $total_data[] = array( 
						'code'       => 'salasa.salasa',
		        		'title'      => $this->language->get('text_description'),
		        		'text'       => $this->currency->format('n\a'),
		        		'value'      => 'n\a',
						'sort_order' => 2
					);
		        }
