...
class ModelShippingSellerBased extends Model {
	private $type = 'shipping';
	private $name = 'seller_based';
	private $charge;
	
	public function getQuote($address,$seller_id = null,$totalCost = null) {

		$settings = $this->getSettings($seller_id);
		...
		foreach (array('shipping', 'payment') as $address_type) {
			if (empty($address) || $address_type == 'payment') {
				...
				if (!empty($this->session->data['area_id']))							$address['area_id'] = $this->session->data['area_id'];
				...
				if (!empty($this->session->data[$address_type . '_area_id']))			$address['area_id'] = $this->session->data[$address_type . '_area_id'];
			...
			if (empty($address['area_id']))		$address['area_id'] =  $this->config->get('config_area_id');
			...
			$geo_zones_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE country_id = " . (int)$address['country_id'] . " AND (zone_id = 0 OR zone_id = " . (int)$address['zone_id'] . ") AND (area_id = 0 OR area_id = " . (int)$address['area_id'] . ")");
		...
		foreach ($settings['charge'] as $charge) {
            ...
            $getTitle= $this->getOptionName($charge['title_admin']);      
			$charge['title_' . $language]= $getTitle['title'];
			...
			elseif ($charge['type'] == 'percentage') {	
				...
				$chargesArr = explode(":", $charge['charges']);
				$percentage = $chargesArr[0];
				$rangeLessThan = $chargesArr[1];
				$rangeGreaterThan = $chargesArr[2];
				if($totalCost <= $rangeLessThan && $totalCost >= $rangeGreaterThan )
				{
				   $cost= $totalCost * (float)$percentage;
				   $rate_found = true;
				}
				else{
					$rate_found = false;
	...
	// Private functions
	//------------------------------------------------------------------------------
	private function getSettings($seller_id) {	
		
		$settings = array();
		$settings2 = array();
		$final_settings = array();
		$settingsSeller = array();
		$sellerBasedSettingsConfigs= $this->config->get('seller_based_charge_'.$seller_id);
		
		foreach ($sellerBasedSettingsConfigs as $key => $value) {
		...
		if ($sellerBasedSettingsConfigs['admin_defult_charges'])
		{
			$defultOption_ids=$sellerBasedSettingsConfigs['admin_defult_charges'];
			
			foreach( $defultOption_ids as $option_id)
			{
			$sellerBasedSettingsDefultConfig= $this->config->get('seller_based_charge_admin_'.$option_id);
			
		   foreach ($sellerBasedSettingsDefultConfig as $key => $value) {

			$split_key = preg_split('/_(\d+)_?/',$key, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
				
			if (count($split_key) == 1)	$settings2[$split_key[0]] = $value;
			elseif (count($split_key) == 2)	$settings2[$split_key[0]][$split_key[1]] = $value;
			elseif (count($split_key) == 3)	$settings2[$split_key[0]][$split_key[1]][$split_key[2]] = $value;
			elseif (count($split_key) == 4)	$settings2[$split_key[0]][$split_key[1]][$split_key[2]][$split_key[3]] = $value;
			else 							$settings2[$split_key[0]][$split_key[1]][$split_key[2]][$split_key[3]][$split_key[4]] = $value;
			}
		//	echo '<pre>';print_r($settings['charge']);exit;
		}
		$settings['charge']=array_merge($settings['charge'],$settings2['charge']);
	}
	...
	public function ruleViolation($rule, $value) {
		$violation = false;
    ...
    public function getMultiSellerOptions($default)
    {
        $queryString = 'SELECT * FROM `ms_sellerbased_options_description` as d';
        $queryString .= ' INNER JOIN `ms_sellerbased_options` as o';
        $queryString .= ' ON d.sellerbased_options_id =o.sellerbased_options_id ';
        $queryString .= ' WHERE o.sellerbased_options_status=1
		                  AND o.sellerbased_options_default='.$default.'
    ...
    public function getOptionName($id){
         $query = $this->db->query("SELECT title FROM `" . DB_PREFIX . "ms_sellerbased_options_description` WHERE sellerbased_options_id = '" . $id . "' AND language_id = '".$this->config->get('config_language_id')."'");
         return $query->row;
	}

	public function getSellerCartProductsData()
	{
		$cart_products=$this->cart->getProducts();
		$total_cost=0;
		...
		{ 
			...
			$cart_product_seller_name=$this->MsLoader->MsProduct->getSellerName($cart_product_seller_id);
			$cartproduct['seller'] = $cart_product_seller_id;
			$pr_group[] = $cartproduct;		
			$sellernames[$cart_product_seller_id]['seller_name'] = $cart_product_seller_name;
			$sellerIDs[] = $cart_product_seller_id;
			$sellerProductsObject[$cart_product_seller_id]['products'][] = $cartproduct;			
																		
		}
		foreach ($sellerProductsObject as $key => $seller_prd) {
            // SUM products price for the same seller
			$totalCost = 0;
			$totalAfterShippingCostForAllSellers=0;
            foreach ($seller_prd['products'] as $prd) {
				$totalCost +=$this->tax->calculate($prd['price'], $prd['tax_class_id'], $this->config->get('config_tax')) * $prd['quantity'];
			}
			$sellerProducts_price[$key] = $this->currency->format($totalCost);
			$sellerShippingMethods[$key]= $this->get_seller_shipping_methods($this->session->data['shipping_address'],$key,$totalCost);	
           
			if(isset($this->request->post['shipping_method_'.$key])){
				$selectedShippingMethod[$key]= $this->request->post['shipping_method_'.$key];
			}else{
			$selectedShippingMethod[$key]='seller_based.seller_based_0_'.$key;
			}		
			$shipping = explode('.', $selectedShippingMethod[$key]);
			$shipping[1]= str_replace("_".$key,"",$shipping[1]);
			$shippingMethod = (isset($sellerShippingMethods[$key][$shipping[0]]['quote'][$shipping[1]])) ? $sellerShippingMethods[$key][$shipping[0]]['quote'][$shipping[1]] : 0;
			$selectedShippingMethodName[$key]=$shippingMethod['title'];
			$selectedShippingMethodValue[$key]=$shippingMethod['text'];

			$totalAfterShippingCost[$key]=$this->currency->format($shippingMethod['cost']+$totalCost);	
			$totalshipping[$key] =$shippingMethod['cost'];
			$totalShippingCostForAllSellers =array_sum($totalshipping);		
			
		}
		$sellerCartProductsData = [
			'products'  => $pr_group,
			'seller_names'  => $sellernames,
			'seller_ids'  => $sellerIDs,
			'products_total_Price'=>$sellerProducts_price,
			'seller_shipping_methods'=>$sellerShippingMethods,
			'selectedShippingMethodName'=>$selectedShippingMethodName,
			'selectedShippingMethodValue'=>$selectedShippingMethodValue,
			'selectedShippingMethod'=>$selectedShippingMethod,
			'totalAfterShippingCost'=>$totalAfterShippingCost,
			'totalShippingCost'=>$totalShippingCostForAllSellers,
		];
		return $sellerCartProductsData;
	}

	private function get_seller_shipping_methods($shipping_address,$seller_id,$totalCost){
		$quote_data = array();
		$code='seller_based';
        $settings = $this->config->get($code);
            ...
            if ($status == 1) {
				$quote = $this->getQuote($shipping_address,$seller_id,$totalCost);
	...
	public function addShipmentDetails($orderId, $details, $status)
    {
        ...
        $fields[] = 'shipment_operator="seller_based"';
        $query[] = implode(', ', $fields);
        $this->db->query(implode(' ', $query));
	}
	public function getShipmentDetails($orderId) {
        $result = $this->db->query("SELECT * FROM  " . DB_PREFIX . "shipments_details where `order_id` = $orderId AND `shipment_operator` = 'seller_based' ");
        return $result->row;
    }
}
