...
class ModelShippingAyMakan extends Model {
	...
	function getQuote($address){
		...
		//Get Title
		$this->language->load_json('shipping/aymakan');
		$title = $this->config->get('aymakan_display_name')[$this->config->get('config_language_id')] ?: $this->language->get('title');


		//Calculate Price
		$settings['aymakan_price'] = $this->config->get('aymakan_price');
    ...
    $cart_weight = $this->cart->getWeight();
		...
		foreach ($geo_zones->rows as $geo_zone) {
			...
			if ($settings['aymakan_price']['aymakan_weight_' . $geo_zone['geo_zone_id'] . '_status']) {
				...
				if($client_geo_zones->num_rows){
					...
					//calculate price
					$cost = '';
                	$rates = explode(',', $settings['aymakan_price']['aymakan_weight_' . $geo_zone['geo_zone_id'] . '_rate']);
					...
					if($right_weight == null || $right_cost == null){
						$total_shipping_cost = $settings['aymakan_price']['aymakan_weight_general_rate'];
					}
					...
					$quote_data['aymakan'] = array(
                        'code'         => 'aymakan.aymakan_weight_' . $geo_zone['geo_zone_id'],
						...
						'tax_class_id' => $this->config->get('aymakan_tax_class_id'),
		...
		if ($status) {
			...
			$quote_data['aymakan'] = array(
				'code'         => 'aymakan.aymakan',
				'title'        => $title,
				'cost'         => $settings['aymakan_price']['aymakan_weight_general_rate'],
				'tax_class_id' => $this->config->get('aymakan_tax_class_id'),
				'text'         => $this->currency->format($settings['aymakan_price']['aymakan_weight_general_rate'])
			);
