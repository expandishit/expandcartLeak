...
class ModelShippingNyazik extends Model {
	...
	public function getQuote($address){
		...
		$this->language->load_json('shipping/nyazik');
		...
		foreach ($geo_zones->rows as $geo_zone) {
			...
			if ($settings['price']['nyazik_geo_zone_id_' . $geo_zone['geo_zone_id'] . '_status']) {
				...
				if($client_geo_zones->num_rows){
					//calculate price
                	$rates = explode(',', $settings['price']['nyazik_geo_zone_id_' . $geo_zone['geo_zone_id'] . '_rate']);
					...
					if($right_weight == null || $right_cost == null){
						$total_shipping_cost = $settings['price']['nyazik_general_rate'];
					}
					else{ //cost&wright are found..
						$total_shipping_cost = $right_cost;
					}

					$quote_data['nyazik_weight_' . $geo_zone['geo_zone_id']] = array(
                        'code'         => 'nyazik.nyazik_weight_' . $geo_zone['geo_zone_id'],
						...
						'tax_class_id' => $this->config->get('nyazik_tax_class_id'),
	...
	private function _getTitle(){
		return $this->config->get('nyazik')['display_name'][$this->config->get('config_language_id')] ?: $this->language->get('nyazik_title');
	}
}
