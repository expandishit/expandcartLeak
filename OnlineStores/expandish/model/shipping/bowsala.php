...
class ModelShippingBowsala extends Model {
	...
	public function getQuote($address){
		$this->language->load_json('shipping/bowsala');
		$settings['bowsala_price'] = $this->config->get('bowsala_price');
		...
		foreach ($geo_zones->rows as $geo_zone) {
            if ($settings['bowsala_price']['bowsala_weight_' . $geo_zone['geo_zone_id'] . '_status']) {
                ...
                if ($client_geo_zones->num_rows) {
                    ...
                    $rates = explode(',', $settings['bowsala_price']['bowsala_weight_' . $geo_zone['geo_zone_id'] . '_rate']);
                    ...
                    // 	$total_shipping_cost = $settings['bowsala_price']['bowsala_weight_rate_class_id'];
                    // }
                    ...
                    $quote_data['bowsala'] = array(
                        'code' => 'bowsala.bowsala_weight_' . $geo_zone['geo_zone_id'],
                        'title' => $this->language->get('bowsala_title') . ' (' . $this->language->get('text_weight') . ' ' . $this->weight->format($cart_weight, $this->config->get('config_weight_class_id')) . ')',
                        'cost' => $right_cost,
                        // 'tax_class_id' => 0,
		...
		if ($status) {
			...
			$method_data = array(
				'code'       => 'bowsala.bowsala',
				'title'      => $this->language->get('bowsala_title'),
				'quote'      => $quote_data,
				'sort_order' => 0,
				'error'      => false
			);
		    ...
		    if($disable_zone==1)
            {
                //to exclude disable zone from work with bowsala shipping menthod
                $method_data=array();
            }else {
                //no zone found
                $quote_data['bowsala'] = array(
                    'code' => 'bowsala.bowsala',
                    'title' => $this->language->get('bowsala_title'),
                    'cost' => $settings['bowsala_price']['bowsala_weight_rate_class_id'],
                    // 'tax_class_id' => 0,
                    'text' => $this->currency->format($settings['bowsala_price']['bowsala_weight_rate_class_id'])
                );
