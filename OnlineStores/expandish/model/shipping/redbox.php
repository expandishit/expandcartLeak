...
class ModelShippingRedbox extends Model {
	...
	function getQuote($address){
		...
		//Get Title
		$this->language->load_json('shipping/redbox');
		$title = $this->config->get('redbox_display_name')[$this->config->get('config_language_id')] ?: $this->language->get('title');


		//Calculate Price
		$settings['redbox_price'] = $this->config->get('redbox_price');
		...
		foreach ($geo_zones->rows as $geo_zone) {
			...
			if ($settings['redbox_price']['redbox_weight_' . $geo_zone['geo_zone_id'] . '_status']) {
				...
				if($client_geo_zones->num_rows){ 
					...
					//calculate price
					$cost = '';
                	$rates = explode(',', $settings['redbox_price']['redbox_weight_' . $geo_zone['geo_zone_id'] . '_rate']);
					...
					if($right_weight == null || $right_cost == null){
						$total_shipping_cost = $settings['redbox_price']['redbox_weight_general_rate'];
					}
					...
					$quote_data['redbox'] = array(
                        'code'         => 'redbox.redbox_weight_' . $geo_zone['geo_zone_id'],
						...
						'tax_class_id' => $this->config->get('redbox_tax_class_id'),
		...
		if ($status) {
			...
			$quote_data['redbox'] = array(
				'code'         => 'redbox.redbox',
				'title'        => $title,
				'cost'         => $settings['redbox_price']['redbox_weight_general_rate'],
				'tax_class_id' => $this->config->get('redbox_tax_class_id'),
				'text'         => $this->currency->format($settings['redbox_price']['redbox_weight_general_rate'])
			);
