...
class ModelShippingUps extends Model {
	function getQuote($address) {
		$this->language->load_json('shipping/ups');
		...
		if ($status) {
            ...
            if($this->config->get('config_language') !== 'en'){
                ...
                if($languages['en']['language_id']) {
                    $languageID = $languages['en']['language_id'];
                }
            }

			$length_code = strtoupper($this->length->getUnit($this->config->get('ups_length_class_id'), $languageID));
