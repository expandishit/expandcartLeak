...
class ModelCheckoutVoucher extends Model {
	...
	public function getVoucher($code) {
		...
		if ($status) {
			return array(
				...
				'amount'           => $amount * ($voucher_query->row['quantity'] ? $voucher_query->row['quantity'] : 1),
	...
	public function confirm($order_id) {
		...
		if ($order_info) {
			...
			$language->load_json($order_info['language_filename']);
			$language->load_json('mail/voucher');
			...
			foreach ($voucher_query->rows as $voucher) {
				...
				if (\Filesystem::isExists('image/' . $voucher['image'])) {
                    $template->data['image'] = \Filesystem::getUrl('image/' . $voucher['image']);
					// $template->data['image'] = $this->config->get('config_url') . 'image/' . STORECODE . '/' . $voucher['image'];
				} else {
					$template->data['image'] = '';
				}
