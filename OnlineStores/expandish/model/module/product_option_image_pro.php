...
class ModelModuleProductOptionImagePro extends Model
{
    ...
    public function getProductOptionImages($product_id, $return_table = false, $checkQuantity = true)
    {
        ...
        $queryCheck = $this->db->query('SHOW TABLES LIKE "' . DB_PREFIX . 'poip_option_image"');
        if($queryCheck->num_rows){
            $quantityQuery = '';
            if ($checkQuantity)
                $quantityQuery = 'AND (NOT POV.subtract OR POV.quantity > 0)';
    ...
    public function addOptionImagesToProductImages($product_images_old, $product_id, &$product_images)
    {
        ...
        if ($this->installed() && isset($poip_settings['img_main_to_additional']) && $poip_settings['img_main_to_additional']) {
                ...
                foreach ($image_po as $product_option_value_id => $image_pov) {
                    if (count($image_pov) > 0) {
                        ...
                        $category_images[$product_option_id][$product_option_value_id] = array(
                            'icon' => $this->model_tool_image->resize($image_pov[0]['image'], $icon_width, $icon_height)
    ...
    public function saveProductOptionValueImages($product_id, $product_option_id, $product_option_value_id, $images)
    {
        ...
        if (is_array($images)) {
            foreach ($images as $image) {
                if (is_array($image) && isset($image['image']) && !empty($image['image'])) {
                    $imagePath = $this->resolveImagePath($image['image']);
                    $this->db->query("INSERT INTO ".DB_PREFIX."poip_option_image
                        SET product_id = ".(int)$product_id."
                        , product_option_id = ".(int)$product_option_id."
                        , product_option_value_id = ".(int)$product_option_value_id."
                        , image = '".$this->db->escape((string)$imagePath)."'
    ...
    /**
     * Check if file is tmp if tmp
     * move it to seller images dir
     *
     * @param string $imagePath
     * @return string $imagePath
     */
    private function resolveImagePath(string $imagePath)
    {
        $tmpPath = $this->config->get('msconf_temp_image_path'). basename($imagePath);
 
        if(\Filesystem::isExists('image/' . $tmpPath)) {
            $imagePath = $this->MsLoader->MsFile->moveImage($imagePath);
        }
            
        return $imagePath;
    }

    public function getProductOrderImage($product_id, $option_data, $image, $checkQuantity = true) {
        ...
        $product_images = $this->getProductOptionImages($product_id, false, $checkQuantity);
        if ( count($product_images) > 0 ) {
