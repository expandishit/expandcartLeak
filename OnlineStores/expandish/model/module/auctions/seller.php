...
class ModelModuleAuctionsSeller extends Model
{

        public function getAll($seller_id){
        $sql="SELECT a.* , pd.name as product_name
                FROM `" . DB_PREFIX . "auction` a
                JOIN `" . DB_PREFIX . "product_description` pd
                    ON pd.product_id = a.product_id
                WHERE pd.language_id = " . (int)$this->config->get('config_language_id'). " AND a.seller_id=".$seller_id."";
        ...
        public function getAuctionProducts($seller_id){
        return $this->db->query("SELECT p.product_id, pd.name, p.quantity, p.price 
        FROM " . DB_PREFIX . "product p 
        LEFT JOIN " . DB_PREFIX . "product_description pd 
            ON (p.product_id = pd.product_id)
        LEFT JOIN " . DB_PREFIX . "ms_product msp
        ON (p.product_id = msp.product_id)
        WHERE pd.language_id = " . (int)$this->config->get('config_language_id') . " AND msp.seller_id=".$seller_id."
        ...
        public function add($auction,$seller_id){
        ...
        public function update($auction,$seller_id){
        ...
        return $this->db->query( "DELETE FROM `".DB_PREFIX."auction_bid` WHERE bid_id = " . (int) $bid_id . " " );

        // return false;
        }
        public function getAuctionsOrders($seller_id){
            return $this->db->query("SELECT * , CONCAT(c.firstname , ' ' , c.lastname ) as customer_name
                ...
                WHERE order_id != 0 AND a.seller_id  = " . (int) $seller_id . " ; ")->rows;
        }
}
?>