...
class ModelAccountSignup extends Model {
	...
	public function isActiveMod() {
       // $this->install();
        ...
        if($exist_table->num_rows == 0 || !\Extension::isInstalled('signup')){
            return false;
        }
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "signupkw LIMIT 1");
		return ($query->row && $query->row['enablemod']) ? $query->row : 0;
    }
    ...
    public function isCountriesPhonesCodes()
    {
        # code...
        if(!$this->isActiveMod()){
            return false;
        }
        $query = $this->db->query("SELECT country_phone_code FROM " . DB_PREFIX . "signupkw LIMIT 1");
        return $query->row['country_phone_code'];
    }
    ...
    // get password min and max length
    public function getPasswordMinMax()
    {
        if(!$this->isActiveMod()){
            return false;
        }

		$query = $this->db->query("SELECT pass_min, pass_max FROM " . DB_PREFIX . "signupkw_attributes LIMIT 1");
		return $query->row;
    }

    public function gatLocals($langId){

        $check_table_exists=$this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "signupkw_locales'" );

        if($check_table_exists->num_rows > 0){
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "signupkw_locales WHERE language_id ='".$langId."'");
            return $query->rows;
        }
        else{
            return array();
    ...
    public function getModLocal($langId,$key) {
        if(!$this->isActiveMod()){
            return false;
        }
    ...
    public function isCountriesPhonesCodesLogin()
    {
        # code...
        if(!$this->isActiveMod()){
            return false;
        }
        $query = $this->db->query("SELECT country_phone_code_login FROM " . DB_PREFIX . "signupkw LIMIT 1");
        return $query->row['country_phone_code_login'];
    }
}
?>