	...
	$('#tab-options').load($('base').attr('href') + 'index.php?route=seller/account-product/jxRenderOptions&product_id=' + msGlobals.product_id, function(data){
		...
		if (msGlobals.product_id.length > 0) {
			...
			$.get($('base').attr('href') + 'index.php?route=seller/account-product/jxRenderProductOptions&product_id=' + msGlobals.product_id, function(data) {
				...
				if(data){
					$('#id-options-sku').fadeIn(100);
					// load options sku tab
					$('#tab-options-sku').load($('base').attr('href') + 'index.php?route=seller/account-product/jxRenderOptionsSKU&product_id=' + msGlobals.product_id, function(data){
						// load existing product options
						if (msGlobals.product_id.length > 0) {
							$.get($('base').attr('href') + 'index.php?route=seller/account-product/jxRenderProductOptionsSKU&product_id=' + msGlobals.product_id, function(data) {
								$('div.options-sku').append(data);
							});
						}
					});
				}
