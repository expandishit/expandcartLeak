$(function() {
    ...
    $('input[name=\'product_manufacturer\']').autocomplete({
        ...
        source: function(request, response) {
            $.ajax({
                ...
                data: {'type':'manufacturers', 'filter_name': request.term},
	...
	$(".ms_product_download_files").delegate(".ms-button-delete", "click", function() {
		$(this).parents('.ms-download').remove();
		return false;
	});
	...
	$("#ms-submit-button").click(function() {
        ...
        var values = $('#ms-new-product').serializeArray();
        
        var images = $('.ms-image');
        
        images.each((i, imgContainer) => {
            var image = [],
                optionId = parseInt(imgContainer.querySelector('input.product_option_id').value),
                optionValueId = parseInt(imgContainer.querySelector('input.product_option_value_id').value),
                imgPath = imgContainer.querySelector('input.product_image').value;
            
            if (optionId) {
                var allOptsIdsKeys = values.filter(function (value) {
                    var rgx = value && value.name.match(/product_option\[[0-9]]\[option_id]+/g);
                    return rgx && rgx.length > 0 && value.value == optionId;
                });
                if (allOptsIdsKeys.length) {
                    var allOptsValIdsKeys = values.filter(function (value) {
                        var rgx = value.name.match(/product_option_value]\[[0-9]]\[option_value_id]+/g);
                        return rgx && rgx.length > 0 && value.value == optionValueId;
                    });
                    
                    if (allOptsValIdsKeys.length) {
                        var optPos = allOptsIdsKeys[0].name.match(/[0-9]+/g).shift();
                        var optValuePos = allOptsValIdsKeys[0].name.substr(30).match(/[0-9]+/g).shift();
                      
                        image = [
                            { name: 'product_option[' + optPos + '][product_option_value]['+ optValuePos +'][images][' + (i) + '][image]', value: imgPath },
                            { name: 'product_option[' + optPos + '][product_option_value]['+ optValuePos +'][images][' + (i) + '][srt]', value: i },
                        ]
                    }
                ...
                image = [
                    { name: 'product_image[' + i + '][image]', value: imgPath },
                    { name: 'product_image[' + i + '][sort_order]', value: i },
                ];
            }
			...
			FileUploaded: function(up, file, info) {
                ...
                if (!$.isEmptyObject(data.files)) {
					for (var i = 0; i < data.files.length; i++) {
						...
						'<div class="ms-image">' +
						'<input type="hidden" class="product_image" value="'+data.files[i].name+'"  />' +
						'<input type="hidden" class="product_option_id" value="0"  />' +
						'<input type="hidden" class="product_option_value_id" value="0"  />' +
                            '<img src="' + data.files[i].thumb + '" />' +
                            (
                                (function () {
                                    if (parseInt(msGlobals.poip_installed) === 1 && parseInt(msGlobals.product_id) > 0) {
                                        var selectMarkup = '<select class="thumb-option-selector" onchange="handleImageOptionChange(this)">' +
                                            '</select>';
	...
	new plupload.Uploader($.extend(true, uploaderParams, {
		browse_button: 'ms-product-addfiles',
		url: $('base').attr('href') + 'index.php?route=seller/account-product/jxUploadProductFile',
		
		preinit : {
			Init: function(up, info) {
				$(".product_download_files").attr("id", up.id);
				$(".download.progress.product_file, #error_product_file_download").addClass(up.id);
			}
		},
		...
		init : {
			FileUploaded: function(up, file, info) {
				...
				if (!$.isEmptyObject(data.files)) {
					for (var i = 0; i < data.files.length; i++) {
						$(".ms_product_download_files").html(
							'<div class="ms-download">' +
			  				'<input type="hidden" name="product_file" value="' + data.files[i].fileName + '" />' +
			  				'<span class="ms-download-name">'+data.files[i].fileMask+'</span>' +
				  			'</div>');
					}
