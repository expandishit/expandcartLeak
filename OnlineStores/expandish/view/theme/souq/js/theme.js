(function($){
	...
	$(".owlCarousel-category").owlCarousel({
	...
	if($(window).width()>480){
		$(window).scroll(function(){
			...
			$('.main-header6').addClass('fixed-header');
			}
			else {
			$('.main-header6').removeClass('fixed-header');
			}
		});
	}

	$('.gallery-without-sidebar').on('click',function(event){
		event.preventDefault();
	});
	...
	//Menu Responsive
	function rep_menu(){
		$(window).on('click','.toggle-mobile-menu',function(event){
			event.preventDefault();
	...
	jQuery(document).ready(function(){
		//Detail Gallery
      // Activate clicking pictures in phone mode
		// if (window.matchMedia("(max-width: 768px)").matches) {
		// 	/* The viewport is less than, or equal to, 700 pixels wide */
		// 	$('.carousel.flexslider').flexslider({
		// 		useCSS: false
		// 	});
		// 	$('.carousel.flexslider ol.flex-control-paging').remove();
		//   }
		//Drop Box Hover
    ...
    if ($('.wrap-item').length > 0) {
      var owl_auto_play = true;
      $('.wrap-item').each(function () {
        var data = $(this).data();

        if (isMobile && data.owlmainslider != '1') owl_auto_play = false;
        else owl_auto_play = true;

        $(this).owlCarousel({
			addClassActive: true,
			stopOnHover: true,
			itemsCustom: data.itemscustom,
			autoPlay: owl_auto_play,
    ...
    // EC-22998  Pop up window appear at end of page on iphone broswer

    if (navigator.userAgent.match(/(iPod|iPhone|iPad)/)) {
      if ($('.fancybox-overlay').has('.fancybox-wrap').length === 0) {
	...
	$("body").on('click','.fancybox-button--zoom', function(){
		$('.fancybox-slide.fancybox-slide--image.fancybox-slide--current.fancybox-slide--complete').toggleClass('around-zoom');
	});
	// Start Around Zoom
	//Window Resize
	...
	//M-Alaa fixed Nav animation.
	$('#canvasTrigger').on('click',function(){
		if ($('#sidebar-left').hasClass("hidden")) {
			$('#canvas').addClass('animation');
		$('#sidebar-left').removeClass('hidden');
		$('#cover').removeClass('hidden').delay('350').queue(function(next){
			$('#sidebar-left').css('z-index', '99999');
			next();
		});
		...
		$('#cover').addClass('hidden').delay('350').queue(function(next){
			$('#sidebar-left').css('z-index', '99999');
			next();
		});
	...
	$('#cover').on('click',function(){
		$('#canvas').removeClass('animation');
		$('#sidebar-left').css('z-index', 'unset');
		$(this).removeClass('animation-cover').delay('400').queue(function(next){
			$('#sidebar-left, #cover').addClass('hidden');
			next();
		});
		$('body').removeClass('hidden-scroll');
	});
