      ...
      function poip_option_value_selected(option) {
        ...
        var uniqueImages = [];
          if(images_to_show) {
              ...
              //</ul></div><div class=\"carousel flexslider\"><ul class=\"slides\"</div>
              // Create the new HTML
              var galleryContent = [];
              for (i = 0; i < uniqueImages.length; i++) {
                  // galleryContent = galleryContent + "<li><a data-fancybox=\"images\" href=\"" + uniqueImages[i] + "\"><img style=\" max-height:400px \"src=\"" + uniqueImages[i] + "\"/></a></li>"
                  galleryContent = galleryContent + `<li><img  class='product-zoom productZoomData' data-zoom-auto="${uniqueImages[i]}" data-src="${uniqueImages[i]}" data-zoom-image="${uniqueImages[i]}"  style=" max-height:400px" src= "${uniqueImages[i]}"/></li>`

              
                }
              $('.gallery-without-sidebar .slider .flex-viewport .slides').empty();
              $('.gallery-without-sidebar .slider .flex-viewport .slides').html(galleryContent);

              var galleryContent2 = [];
              for (i = 0; i < uniqueImages.length; i++) {
                  // galleryContent2 = galleryContent2 + "<li> <a href=\"#\"  data-image=\" " + uniqueImages[i] + "\" data-zoom-image=\" " + uniqueImages[i] + "\"><img src=\"" + uniqueImages[i] + "\"></a></li>"
                  galleryContent2 = galleryContent2 + `<li><a href="#" data-image="${uniqueImages[i]}" data-zoom-image="${uniqueImages[i]}"> <img  src= "${uniqueImages[i]}"/></a></li>`
                }
              $('.gallery-without-sidebar .carousel .flex-viewport .slides').empty(); 
              $('.gallery-without-sidebar .carousel .flex-viewport .slides').html(galleryContent2);

              // add active style for the first thumbnail
              $('.carousel li:first-child').addClass('flex-active-slide');
              ...
              $(".product-zoom.productZoomData").elevateZoom();
          }
      }
