...
<div id="content" class="ms-account-withdrawal">
	...
	<?php } else if ($msconf_payment_method == 1 && (!isset($paypal) || empty($paypal)) ) { ?>
	...
	<form id="ms-withdrawal" class="ms-form">
		<div class="content">
			<?php if (!$withdrawal_minimum_reached || ($msconf_payment_method == 1 && (!isset($paypal) || empty($paypal))) || $balance_available <= 0) { ?>
			...
			<table class="ms-product">
				...
				<tr>
					...
					<td>
                        <?php if ($msconf_payment_method == 1){ ?>
						<p>
                        ...
                        <?php if ($msconf_payment_method == 2){ ?>
                            <p>
                                <input type="radio" name="withdraw_method" value="bank_transfer" checked="checked" />
                                <span><?php echo $ms_payment_method_bank; ?></span>
                            </p>
                        <?php } ?>
		...
		<div class="buttons">
			...
			<?php if ($withdrawal_minimum_reached && isset($paypal) && !empty($paypal) || ($withdrawal_minimum_reached && $msconf_payment_method == 2)) { ?>
			<div class="right">
