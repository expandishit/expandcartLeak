		...
		<div class="wrapper">
			...
			<?php foreach($subscriptions['plans'] as $key => $plan) { ?>
				<label class="package">
					...
					<div class="discription"><?php echo html_entity_decode($plan['description']); ?></div>
					<?php /* ?>
...
<script type="text/javascript">
	...
	$('#ms-submit-button').click(function (e) {
        ...
        var data = $('#ms-subscriptions').serialize() + '&seller[price]=' + planDetails.data('amount') +
			'&seller[isRecurring]=' + (isRecurring ? '1' : '0') + '&seller[newSeller]=true';
			...
			success: function(response) {
			    ...
			    if (response.errors) {
                    ...
                    if (paymentMethod === '1' && parseFloat(planDetails.data('usdamount')) > 0 ) {
                        ...
                        $('#cmd').val(isRecurring ? '_xclick-subscriptions' : '_cart');
