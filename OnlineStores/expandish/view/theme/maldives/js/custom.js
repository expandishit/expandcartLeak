...
jQuery(function($j) {
	...
	if ($j('.content--parallax').length) {
		...
		$j('.content--parallax').each(function() {
			...
			$j(this).css({'background-image': 'url('+attr+')'}).parallax("50%", 0.0);
...
jQuery(function($j) {
	...
	//For Switched  Mute Video
	const ValForVideo = $('#val_video').val();	
	setTimeout(function(){
		if(ValForVideo == "1"){
			$j("#video_live").prop('muted', true);
		}else{
			$j("#video_live").prop('muted', false);
		}
		
	  },4000); 
	
//  Feed instagram 
const AccessToken = $j('.section-instagram #access_Token').val();
const maxFeeds = $j('.section-instagram #max_feeds').val();
if (AccessToken) {
	jQuery.fn.spectragram.accessData = {
		accessToken: AccessToken    
	};
	$j('.instagram-feeds').spectragram({
		complete:'',
		max: maxFeeds,
		size: "small",
		wrapEachWith: "<div class='box-instagram-feeds'></div>"
	});
}


// To Do 
// $('.instagram-feeds').on('click',".box-instagram-feeds a", function(e){
//     e.preventDefault();
//     console.log('sdsdsd')
//     const Image = $(this).find('img').attr('src');
//     const Paragraph = $(this).attr('title');
//     $('#image_inst_popups').attr('src', Image);
//     $("#parg_inst_popups").append(Paragraph);
//     $('.instagram-feeds').addClass('animation-inst').delay().queue(function(){
//         $('.instagram-feeds').css('opacity',"0")
//         $(".Popups-instagram").addClass('show-animation')
//     })
//     $('.Popups-instagram').addClass('show').delay().queue(function(){        
//         $(".Popups-instagram").addClass('show-animation')
//     })    
// })
//*  End Feed instagram */
