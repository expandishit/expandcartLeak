(function($){
	...
	// set responsive nav height
	$('.push-sidebar').height($(window).height())
	
	// facebook link in touch screens
	if ("ontouchstart" in document.documentElement)
    {
        let fbLink = $('#footer .list-social > a.fb-icon').attr('href');
        $('#footer .list-social > a.fb-icon').attr('href', 'fb://facewebmodal/f?href=' + fbLink)
    }
	...
	jQuery(window).on('load',function(){ 
		...
		if($('.wrap-item').length>0){
			$('.wrap-item').each(function(){
				if ($(this).parent().hasClass('banner-slider'))
				{
					if ($(this).find('.item-banner').length < 2)
					{
						return;
					}
				}
