      ...
      function poip_option_value_selected(option) {
        ...
        if(uniqueImages.length < 3){ $('.flex-next , .flex-prev').hide();}
        else{ 
            $('.flex-next , .flex-prev').show();
        }

        // Create the new HTML
        var galleryContent = [];
        for(i=0;i<uniqueImages.length;i++){
         galleryContent = galleryContent + "<li><a href=\""+uniqueImages[i]+"\"><img style=\" max-width:100px \" src=\""+uniqueImages[i]+"\"   data-cloudzoom=\"useZoom: '#zoom1', smallImage: \'"+uniqueImages[i]+"\' \"></a></li>"
        }
        
        $('.flexslider-thumb').removeData("flexslider");
        $('.flexslider-thumb ul.previews-list').empty();
        $('.flexslider-thumb ul.previews-list').html(galleryContent);

        // //pass the first image to the zoom preview - OK
        $('.zoom-img').attr("src",images_to_show[0]);
        $('#zoom1').attr("src",images_to_show[0]);
        $('#zoom1').attr("data-cloudzoom","zoomImage: '"+images_to_show[0]+"',zoomPosition: 'inside' ,autoInside: true ");
