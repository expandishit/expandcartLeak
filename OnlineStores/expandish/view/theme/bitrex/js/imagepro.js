      ...
      function poip_option_value_selected(option) {
        ...
        if(uniqueImages.length < 3){ $('.gallery-control .prev ,.gallery-control .next').hide();}
        else{ 
            $('.gallery-control .prev , .gallery-control .next').show();
        }

        // Create the new HTML
        var galleryContent = [];
        for(i=0;i<uniqueImages.length;i++){
         galleryContent = galleryContent + "<li style=\"overflow: hidden; float: left; width: 80px; height: 90px;\"><a href=\"#\"  data-image=\" "+uniqueImages[i]+" \" data-zoom-image=\""+uniqueImages[i]+"\"><img src=\""+uniqueImages[i]+"\"></a></li>"
        }
        $('.carousel ul').empty();
        $('.carousel ul').html(galleryContent);
