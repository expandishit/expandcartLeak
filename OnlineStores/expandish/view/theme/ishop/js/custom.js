    ...
    if ($j('.product-zoom').length) {
        ...
        if (windowW > 767) {
			...
			$j('.product-zoom').elevateZoom({
				...
				$(".product-main-image__item").addClass('not-visible')
				$(".product-main-image__item").on("click", function() {
				...
				$(".product-zoom").on("touchmove", function(e) {
					$(this).parent().addClass('not-visible');
					console.log('toutch move')
					$('.zoomWindowContainer > div').hide();
				});
	
				$(".product-zoom").on("touchstart", function(e) {
					$(this).parent().removeClass('not-visible');
				});
        }
...
jQuery(function($j) {
    ...
    $j('.btn-number').click(function(e) {
        ...
        if (!isNaN(currentVal)) {
			...
			if (currentVal > input.attr('min')) {
                ...
                if (parseInt(input.attr('is-unlimited')) == 1 || currentVal < input.attr('max')) {
    ...
    $j('.input-number').change(function() {
        ...
        var minValue = parseInt($j(this).attr('value'));

        var maxValue = parseInt($j(this).attr('maxvalue'));

        var isUnlimited = parseInt($j(this).attr('is-unlimited'));
        ...
        if (isUnlimited == 1 || valueCurrent <= maxValue) {
