      ...
      function poip_option_value_selected(option) {
        ...
        if(uniqueImages.length < 3){ $('.owl-prev , .owl-next').hide();}
        else{ 
        ...
        for(i=0;i<uniqueImages.length;i++){
         galleryContent = galleryContent + "<a href=\"\"  data-image=\""+uniqueImages[i] +"\" data-zoom-image=\""+uniqueImages[i]+"\"><img src=\""+uniqueImages[i]+"\"></a>"
        }
        $('#thumbnails').empty();
        $('#thumbnails').html(galleryContent);

        var $owl = $('.thumbnails_carousel');
        ...
        $('.image_preview_container img').attr("src",images_to_show[0]);
        $('.image_preview_container img').data("zoom-image",images_to_show[0]);
        if ($(window).width() > 767) {
