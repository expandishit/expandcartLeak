...
jQuery(document).ready(function(){
	...
	// products counter
	$('.product-cart-counter .product-cart-btn').click(function() {
		let config = $(this).parents('.product-cart-counter');
		let maxlimit = config.data('maxlimit');
		let count = config.data('count');
		let stateDir = $(this).hasClass('up') ? 'up' : 'down';

		if(stateDir === 'up') {
			config.data('count', config.data('count') + 1)
		}
		if(stateDir === 'down' && config.data('count') > 1) {
			config.data('count', config.data('count') - 1)
		}

		$(this).siblings('.product-cart-count').text(config.data('count'))
	})
	//Detail Gallery
...
// HOT CATEGORIES -- View more && less 
 	ViewMore.on('click', function (){
		$(this).prev().removeClass('hidden').addClass('show');		
		$(this).next().removeClass('hidden').addClass('show');
		$(this).addClass('hidden');
	});
	ViewLess.on('click', function (){
		$(this).prev().prev().removeClass('show').addClass('hidden');
		$(this).addClass('hidden');
		$(this).prev().removeClass('hidden').addClass('show');
	});
