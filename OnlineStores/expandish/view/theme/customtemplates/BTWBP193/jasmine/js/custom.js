...
// calculate mega menue posetion in mulible lines case
$('.navbar.navbar-wd .nav > li').mouseenter(function() {
	let parentPosetion = $('.navbar.navbar-wd .nav').offset().top + $('.navbar.navbar-wd .nav').height();
	let itemPosetion = $(this).offset().top + $(this).height();
	let itemTop = (parentPosetion - itemPosetion);
	let menueTop = ($('.navbar.navbar-wd .nav').height() - itemTop) + 'px';
...
$('.quantity-minus').on('click', function(e) {
    ...
    let counterInput = $(`#qty_cat${product_id}`);
    let currentProductCount = parseInt(counterInput.val());

    if (currentProductCount === parseInt(counterInput.prop('min'))) {
      return;
    }

    counterInput.val(--currentProductCount);
});
...
$('.quantity-plus').on('click', function(e) {
    ...
    let product_id = $(this).data('product-id');
    let subtractStock = $(this).data('subtract-stock');
    let counterInput = $(`#qty_cat${product_id}`);
	let currentProductCount = parseInt(counterInput.val());

    if (currentProductCount >= parseInt(counterInput.prop('max')) && subtractStock) {
      return;
    }
