...
/*class ControllerCheckoutCheckoutv2 extends Controller
{
	public function index()
	{
        ...
        $this->data['quickcheckout'] = $this->getChild('module/quickcheckoutv2');

        $this->template = 'default/template/checkoutv2/quickcheckout_main.expand';
...
class ControllerCheckoutCheckoutv2 extends Controller {
    ...
    private $time = '';
    private $countries = [];
    private $checkoutv2_settings = array();
    private $paymentMethodModel;
    public function __construct($registry){
        parent::__construct($registry);
        $this->checkoutv2_settings = $this->config->get('checkoutv2');
        $this->paymentMethodModel = $this->load->model('extension/payment_method', ['return' => true]);
    }
    ...
    public function index() {
        ...
        // To give the customer the freedom to choose between shipping and payment methods
       ...
       // $template = 'quickcheckout';
        ...
        if($this->validateCheckout()) {
           ...
           // }else{
            ...
            $this->data['hide_section_address'] = !empty($this->session->data['subscription']);
            ...
            $this->data['current_step'] = $this->session->data['current_step'];
            ...
            $this->data['checkout_mode'] = $this->checkoutv2_settings['checkout_mode'] ?: 3; // default 3steps UI
            ...
            if ($this->data['checkout_mode'] == 1) {
                $this->session->data['current_step'] = 'all';
            }

            $this->template = 'default/template/checkoutv2/checkoutv2.expand';
    ...
    */
    private function section_address(){
        ...
        $data['country_id']['value'] = $country_id = $this->config->get('config_country_id');
        $data['country_id']['options'] = $this->get_countries();
        $data['zone_id']['value'] = $this->config->get('config_zone_id');
        $data['customer_group_id']['value'] = $this->config->get('config_customer_group_id');
        $data['customer_group_id']['options'] = $this->get_customer_groups();
        ...
        $data['send_as_gift']['status'] = $this->config->get('checkoutv2')['ship_as_gift'] ?? 1;
        if (isset($data['send_as_gift'])) {
            $data['send_as_gift'] = $data['send_as_gift'];
        ...
        $country_data = $this->get_country_data($payment_address['country_id'] ?: $country_id, $payment_address['zone_id']);
        $data['phonecode'] = $country_data['phonecode'];
        $data['flag']      = strtolower($country_data['iso_code_2']);

        //$country_id=$this->config->get('config_country_id');
        /*$query=$this->db->query("SELECT * from ".DB_PREFIX."country where country_id = ".$country_id);
        $result=$query->row;*/
        $this->session->data['country_code'] = $country_data['iso_code_2'] !="" ? $country_data['iso_code_2'] : $country_data['iso_code_3'];    
        ...
        //if($this->customer->isLogged()){
            ...
            if ($this->customer->getId()!=null) {
                ...
                $this->data['addresses'] = $this->model_account_address->getAddress($this->customer->getAddressId());
            }
        ...
        /*if (!$this->cart->hasShipping()) {
            $data['shipping']['value'] = 1;
            $data['shipping']['display'] = 0;
        }*/
        ...
        // print_r($this->data['payment_address']);
        // exit;

        $this->data['checkout_mode'] = $this->checkoutv2_settings['checkout_mode'] ?: 3;
        
        // Google map integration 
        $this->load->model("module/google_map");
        $this->data['google_map'] = $this->model_module_google_map->getSettings();

            
        // form registration & address fields
        $this->data['customer_fields'] = $this->config->get('config_customer_fields');
        
        $this->data['addresses'] = array_map(array($this, 'setExtraParamToAddress'), $this->data['addresses']);
    ...
    private function get_shipping_method_view(){
        //New v 23/02/2020
        $this->data['cart_has_shipping'] = $this->cart->hasShipping();
        $cart_has_product  = $this->cart->hasProducts();

        if(!$this->data['cart_has_shipping']){ return false; }
        ...
        if (!$cart_has_product && !empty($this->session->data['vouchers']) && !empty($this->session->data['subscription'])) {
            $this->data['shipping_methods'] = array();
        }elseif($cart_has_product && !$this->data['cart_has_shipping']){
            $this->data['shipping_methods'] = array();
        }else{
        ...
        $this->data['data'] = [
            ...
            'display_options' => "1",
            'width' => "30",
            'default_option' => "aramex",
            'title' => $this->language->get("option_logged_shipping_method_title"),
            'description' => '',//$this->language->get("text_shipping_method")
        ];
        //print_r($this->settings['option'][$this->session->data['account']]['shipping_method']);
    ...
    private function get_payment_method_view(){
        ...
        $this->data['data'] = [
            ...
            'off_factor_percentage' => "0.00",
            'off_factor_discount_limit' => "0",
            'off_factor_apply_on_shipping' => "0",
            'title' => $this->language->get("option_logged_payment_method_title"),
            'description' => '',//$this->language->get("text_payment_method"),
            'cost' => ""
        ];
    ...
    private function get_cart_view(){
        ...
        if ($cart_products || !empty($this->session->data['vouchers']) || !empty($this->session->data['subscription'])) {
            ...
            */
            ...
            $this->data['products_count'] = $products_count; // count($cart_products) for products count without considering the qauntity
            ...
            if (Extension::isInstalled('reward_points_pro') && $this->config->get('rwp_enabled_module') == "1") {
                $this->data['reward_app'] = 1;

                $this->data['reward_success'] = $this->session->data['reward_success'];
                $this->data['reward_error'] = $this->session->data['reward_error'];
    ...
    private function get_confirm_view(){
        if($this->validateCheckout()){
            ...
            if ($this->session->data['max_quantity_product'] === false) {
                $this->data['button_confirm_display'] = false;
            }
            ...
            $this->data['order_agree'] = $this->config->get('checkoutv2')['order_agree'];
            $this->data['order_agree_confirmed'] = (int)(isset($this->request->post['confirm']['agree']) && $this->request->post['confirm']['agree'] == '1');
            $this->load->model('catalog/information');
            $information_info = $this->model_catalog_information->getInformation($this->config->get('checkoutv2')['information_page_id']);
    ...
    */
    private function get_payment_methods($payment_address){
		...
		if (!empty($this->session->data['subscription']) && isset($method_data['cod'])) {
			unset($method_data['cod']);
		}
        
        // return array_map(
        //     array($this, 'setExtraParamToPaymentMethod'), 
        //     $method_data
        // );
    ...
    // private function setExtraParamToPaymentMethod($method)
    // {
    //     if($payment = $this->paymentMethodModel->selectByCode($method['code'])) {
    //         $method['is_external'] = (int) ($payment['is_external'] ?: 0);
    //     }
    ...
    public function load_settings(){
        ...
        if(!empty($this->request->post)){
            ...
            if (empty($this->request->post['coupon'])) unset($this->request->post['coupon']);
       ...
       // if($this->customer->isLogged()){
            ...
            if($this->session->data['payment_address']['exists']){
                ...
                if (isset($this->request->post['payment_address']['address_id'])) {
                    $addressId = (int)$this->request->post['payment_address']['address_id'];
                                        
                    if ($this->request->post['payment_address']['edit_address'] == 1)
                        $this->model_account_address->editAddress($addressId, $this->request->post['payment_address']);
                    ...
                    unset($this->request->post['payment_address']['edit_address'], $this->session->data['payment_address']['edit_address']);
    ...
    public function confirm_order(){
        ...
        ///Edit address
        elseif($this->session->data['payment_address']['edit_address'] == 1 && isset($this->request->post['payment_address'])){
            $this->model_account_address->editAddress($this->session->data['payment_address']['address_id'], $this->request->post['payment_address']);
        ...
        $this->session->data['payment_address']['firstname'] = $data['payment_firstname'];
        $this->session->data['payment_address']['lastname'] = $data['payment_lastname'];
        $this->session->data['shipping_address']['firstname'] = $data['shipping_firstname'];
        $this->session->data['shipping_address']['lastname'] = $data['shipping_lastname'];
    ...
    */
    public function validate_fields(){
        ...
        if($postData['validate_section'] == 'all'){
            ...
            $error_payment_method  = $this->validate_payment_method($postData);
    ...
    */
    private function validate_payment_address($data){
        ...
        if (!empty($this->session->data['subscription'])) return [];
        ...
        // if($data['address_id'] == "0" || $data['edit_address'] == "1") {
            if($formFields['country_id'] == 1 && !$data['country_id']){
                $json['payment_address']['country_id'] = $this->language->get("error_required_field");
            }
    
            if($formFields['zone_id'] == 1 && !$data['zone_id']){
                $json['payment_address']['zone_id'] = $this->language->get("error_required_field");
            }
            
            if($formFields['area_id'] == 1 && !$data['area_id']){
                $json['payment_address']['area_id'] = $this->language->get("error_required_field");
            }
            ...
            if($formFields['address_1'] && !$data['address_1']){
                $json['payment_address']['address_1'] = $this->language->get("error_required_field");
            }
            
            if($formFields['address_2'] && !$data['address_2']){
                $json['payment_address']['address_2'] = $this->language->get("error_required_field");
            }
            
            if($formFields['postcode'] && !$data['postcode']){
                $json['payment_address']['postcode'] = $this->language->get("error_required_field");
            }
    
            if($formFields['location'] && !$data['location']){
                $json['payment_address']['location'] = $this->language->get("error_required_field");
            }
            
            if($formFields['telephone'] && !$this->simpleValidatePhone($data['telephone'])) {
    ...
    public function simpleValidatePhone(string $phone = null)
    {
    ...
    private function validate_shipping_method($data){
        ...
        if($this->cart->hasShipping()) {
            ...
            if (empty($this->session->data['shipping_method'])) {
                $json['shipping_method']['error_warning'] = $this->language->get("error_required_field");
            }
        }
    ...
    */
    private function validate_payment_method($data=[]){
        ...
        if(empty($this->session->data['payment_method'])){
            $json['payment_method']['error_warning'] = $this->language->get("error_required_field");
        }
        ...
        //Confirm
        if( $this->config->get('checkoutv2')['order_agree'] == 1 && (!isset($data['confirm']['agree']) || $data['confirm']['agree'] != '1')){
    ...
    */
    /*private function validate_confirm($data){
        ...
        //Confirm
        $checkoutv2_settings = $this->config->get('checkoutv2');
        if( $checkoutv2_settings['order_agree'] == 1 && $data['confirm']['agree'] == 0 ){
    ...
    */
    ...
    public function refreshSteps(){
        ...
        for($i = $stepid; $i <= 8; $i++){
            $step = 'refresh_step'.$i;
            $html = method_exists($this, $step) ? $this->$step(1) : null;
    ...
    function shippingAddressLocation($shipping_address){
        if (isset($shipping_address['location']) && count($latlng = explode(',', $shipping_address['location'])) === 2) {
            return [
              'lat' => $latlng[0],  
              'lng' => $latlng[1],  
            ];
    ...
    */
    ...
    public function validate_reward() {
        ...
        if (!isset($json['error'])){
            ...
            $json['success'] = $this->session->data['reward_success'] = $this->language->get('text_reward');
        }else{
    ...
    */
    ...
    public function language_merge_loop($array, $texts){
        ...
        $localization_array = [
            ...
            "entry_city","entry_postcode","entry_country","entry_current_zone"
        ];
    ...
    public function array_merge_recursive_distinct( array $array1, array $array2 ){
        // $merged = $array1;
        // $result = array();
        //$result = $this->cache->get('checkoutv2.array_merge_recursive_distinct.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . md5(serialize($array1)) . '.' . md5(serialize($array2)));
        // if(!$result){
            foreach ($array2 as $key => $value) {
                if (is_array($value) && isset($array1[$key]) && is_array($array1[$key])){
                    // $array1[$key] = $this->array_merge_recursive_distinct_loop($array1[$key], $value);
                    $array1[$key] = $this->array_merge_recursive_distinct($array1[$key], $value);
                }else{
                    $array1[$key] = $value;
                }
            }
            // $this->cache->set('checkoutv2.array_merge_recursive_distinct.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . md5(serialize($array1)) . '.' . md5(serialize($array2)), $array1);
            // $result = $array1;
        // }
        return $array1;
    }
    ...
    public function applyCustomerGroupCoupon()
    {
        if ($this->customer->isLogged()) {
            ...
            if (isset($this->session->data['coupon'])) {
                ...
                $this->validate_coupon(); // if the coupon is a valid add it to session else delete it               
            }
    ...
    private function setCurrenciesData()
    {
    ...
    private function setRedirectUrlData()
    {
        if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
            $connection = 'SSL';
        } else {
    ...
    /**
     * Update current step and save it on session
    */
    public function update_current_step()
    {
        $step = $this->request->post['index'];
        if (isset($step)) {
            if ($step == 0) {
                $this->session->data['current_step'] = "address";
            }
            
            if ($step == 1) {
                if ($this->cart->hasShipping()) {
                    $this->session->data['current_step'] = "shipping";
                } else {
                    $step = 2;
                }
            }
            ...
            if ($step == 2) {
                $this->session->data['current_step'] = "payment";
            }
        }
        
        $this->response->setOutput(json_encode($this->session->data['current_step']));
    }
    ...
    public function updateAddress()
    {
        ...
        $this->response->setOutput(json_encode(['success' => true, 'data' => $this->setExtraParamToAddress($addressModel->getAddress($data['address_id']))]));
        return;
    }
    ...
    public function addAddress()
    {
        ...
        $this->response->setOutput(json_encode(['success' => true, 'data' => $this->setExtraParamToAddress($addressModel->getAddress($address_id))]));
        return;
    }
    ...
    private function validateFormAddress(array &$data)
    {
        ...
        if ($formFields['telephone'] == 1 && !is_simple_valid_phone($data['telephone'])) {
            $errors['telephone'] = $this->language->get('invalid_input_shipping_telephone');
        }
