...
class ControllerCheckoutManual extends Controller {
	public function index() {
		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {	
			...
			$getCurrentCouponHistory = $this->db->query("SELECT * FROM " . DB_PREFIX . "coupon_history INNER JOIN coupon c on coupon_history.coupon_id = c.coupon_id WHERE order_id = '" . (int)$this->db->escape($this->request->post['order_id']) . "'");
			// check if the coupon is different
			if ((empty($getCurrentCouponHistory->row['code']) && !empty($this->request->post['coupon']))
				|| $getCurrentCouponHistory->row['code'] !== $this->request->post['coupon']){
				// delete old coupon history
				if (!empty($getCurrentCouponHistory->row['code']))
					$this->db->query("DELETE FROM " . DB_PREFIX . "coupon_history WHERE order_id = '" . (int)$this->db->escape($this->request->post['order_id']) . "'");

				// if there is new coupon then insert it
				if (!empty($this->request->post['coupon']) && !empty($coupon_info) && $coupon_info['status'])
					$this->db->query("INSERT INTO " . DB_PREFIX . "coupon_history SET order_id = '" . (int)$this->db->escape($this->request->post['order_id']) . "', coupon_id = '" . (int)$coupon_info['coupon_id'] . "', amount = '" . (int)$coupon_info['discount'] . "', customer_id = '".$this->db->escape($this->request->post['order_id']['customer_id'])."', date_added = NOW() ");
			}
