...
class ControllerModuleKnawatDropshipping extends Controller { 
	...
	public function order_changed($data,$is_admin,$new_order=true){
		...
		$expandish_app_dir = str_replace( 'system/', 'expandish/', DIR_SYSTEM );
		
		if($is_admin){
			require_once $expandish_app_dir."model/checkout/order.php";
			require_once $expandish_app_dir."model/account/order.php";
			 ...
			 * Load admin model.
			*/
			require_once $admin_app_dir."model/module/knawat_dropshipping.php";
        ...
        if( !$knawat_order ){
			$this->model_module_knawat_dropshipping->delete_knawat_meta( $order_id, 'knawat_sync_failed', 'order' );
			return true;
		}
		/**
		...
		$mp_order = $this->format_order($is_admin, $order, $order_products, $is_update );
		...
		if( $is_update ){
			...
			if( $failed ){
				$this->model_module_knawat_dropshipping->update_knawat_meta( $order_id, 'knawat_sync_failed', $failed_message, 'order' );
				return false;
			}
		}else{
			$failed = false;
			...
			try{
				...
				if( !empty( $result ) && isset( $result->status ) && 'success' === $result->status ){
					...
					$korder_id = $result->data->id;
					$this->model_module_knawat_dropshipping->update_knawat_meta( $order_id, 'knawat_order','1', 'order' );
					$this->model_module_knawat_dropshipping->update_knawat_meta( $order_id, 'knawat_order_id', $korder_id, 'order' );
					$this->model_module_knawat_dropshipping->delete_knawat_meta( $order_id, 'knawat_sync_failed', 'order' );
	 ...
	 */
	private function format_order($is_admin, $order, $order_products, $is_update = false ){
		...
		$bill_firstname = isset( $order['payment_firstname'] ) && $order['payment_firstname'] ? $order['payment_firstname'] : $order['firstname'] ? $order['firstname'] : '';
		$bill_lastname = isset( $order['payment_lastname'] ) && $order['payment_lastname'] ? $order['payment_lastname'] : $order['lastname'] ? $order['lastname'] : '';
		$billing = array(
			...
			'city' 		=> $order['payment_city'] ?? $order['payment_zone'] ?? '',
			...
			'email' 	=> isset( $order['email'] ) && !empty($order['email']) ? $order['email'] : $this->config->get('config_email'),
			'phone' 	=> isset( $order['telephone'] ) ? $order['telephone'] : ''
		);

		$shipping_firstname = isset( $order['shipping_firstname'] ) && $order['shipping_firstname'] ? $order['shipping_firstname'] : $order['firstname'] ? $order['firstname'] : '';
		$shipping_lastname = isset( $order['shipping_lastname'] ) && $order['shipping_lastname'] ? $order['shipping_lastname'] : $order['lastname'] ? $order['lastname'] : '';
		$shipping = array(
			...
			'city' 		=> isset( $order['shipping_city'] ) && !empty($order['shipping_city']) ? $order['shipping_city'] : '-',
			...
			'email' 	=> isset( $order['email'] ) && !empty($order['email']) ? strtolower($order['email']) : strtolower($this->config->get('config_email')),
			'phone' 	=> isset( $order['telephone'] ) ? $order['telephone'] : ''
		);
		...
		$language = $this->model_localisation_language->getLanguageByCode($language_code);
		...
		if( $payment_code != '' && strtolower($payment_code) == "cod"){
	...
	private function get_knawat_order_product( $order_products, $is_update = false ){
		...
		foreach( $order_products as $product ){
            $order_product_sku = null;
			$is_knawat = $this->is_knawat_product( $product );
			if( $is_knawat ){
				if( $is_update ){
					$order_product_sku = $this->model_module_knawat_dropshipping->get_knawat_meta( $product['order_product_id'], 'sku', 'order_product' );
					if( !$order_product_sku ){
						// Get SKU based on product ID.
                        $order_product_sku = $this->model_catalog_product->getProductCols($product['product_id'], ['sku'])['sku'];
					}
					$temp = array(
					...
					foreach( $product_options as $option ){
						$product_option_sku = $this->model_module_knawat_dropshipping->get_knawat_meta( $option['product_option_value_id'], 'sku', 'product_option_value' );
						if( $product_option_sku != '' ){
							$this->model_module_knawat_dropshipping->update_knawat_meta( $product['order_product_id'], 'sku', $product_option_sku, 'order_product' );
							break;
						}
					}
					if( !$product_option_sku ){
						// Get SKU based on product ID (if its not there for options )
                        $product_option_sku = $this->model_catalog_product->getProductCols($product['product_id'], ['sku'])['sku'];
					}
					$temp = array(
						'quantity'	=> $product['quantity'],
	...
	private function is_knawat_product( $order_product ){
		if( !empty( $order_product ) ){
			...
			if( !isset( $this->model_module_knawat_dropshipping ) ){
				$this->load_admin_model();
			}
			$is_knawat = $this->model_module_knawat_dropshipping->get_knawat_meta( $product_id, 'is_knawat', 'product' );
			if( $is_knawat == '1' ){
				return true;
			}
		}
	 ...
	 */
	public function before_add_to_cart($product_id=''){
		if( isset( $product_id ) && !empty( $product_id) ){
			if( $this->is_knawat_product( (int)$product_id ) ){
				$action_url = $this->url->link( $this->route.'/sync_product_by_id/&product_id='.(int)$product_id );
                $json['success'] = 'Knawat Success';
	...
	public function sync_failed_order(){
		...
		$this->load_admin_model();
		$orders = $this->model_module_knawat_dropshipping->get_sync_failed_orders();
		...
		if( !empty( $orders ) ){
			...
			try{
				...
				foreach( $orders as $order ){
					...
					$result = $this->order_changed($orderdata,false,false);
					if( !$result ){
						$failed = true;
					}
				}
