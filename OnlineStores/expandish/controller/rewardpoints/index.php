<?php
	...
	class ControllerRewardPointsIndex extends Controller
	{
		...
		public function loadBlockRewardPoints()
		{
			...
			//$this->template = 'clearion/template/rewardpoints/checkout/block_rewardpoints.tpl';
            if(file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/rewardpoints/checkout/block_rewardpoints.expand')) {
                $this->template = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/rewardpoints/checkout/block_rewardpoints.expand';
            }
            else {
                $this->template = $this->config->get('config_template') . '/template/rewardpoints/checkout/block_rewardpoints.expand';
            }
			$this->children = array();

			$html_block = $this->render_ecwig();

			$this->load->model('rewardpoints/helper');

			$this->data['totals']   =   $this->model_rewardpoints_helper->collectTotal();

			//$this->template = 'clearion/template/rewardpoints/checkout/cart_total.tpl';
            if(file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/rewardpoints/checkout/cart_total.expand')) {
                $this->template = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/rewardpoints/checkout/cart_total.expand';
            }
            else {
                $this->template = $this->config->get('config_template') . '/template/rewardpoints/checkout/cart_total.expand';
            }
			$this->children = array();

			$html_cart_total = $this->render_ecwig();
		...
		public function getBlockRewardPointsDiscount()
		{
			...
			//$this->template = 'clearion/template/rewardpoints/checkout/order/block_rewardpoints.tpl';
            if(file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/rewardpoints/checkout/order/block_rewardpoints.expand')) {
                $this->template = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/rewardpoints/checkout/order/block_rewardpoints.expand';
            }
            else {
                $this->template = $this->config->get('config_template') . '/template/rewardpoints/checkout/order/block_rewardpoints.expand';
            }
			$this->children = array();
