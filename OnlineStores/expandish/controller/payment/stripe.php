...
class ControllerPaymentStripe extends PaymentController {    
	...
	public function index() {
		...
		$data['checkout_image'] = (!empty($settings['checkout_image'])) ? ( HTTPS_IMAGE.$settings['checkout_image'] ) : '';
		...
		$data['applepay_amount'] = $data['checkout_amount'] / $decimal_factor;
		
		// Render
        // $template_file = '';
        // if(file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/stripe.expand')) {
        //     $template_file = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/stripe.expand';
        // }
        // else {
        //     $template_file = $this->config->get('config_template') . '/template/payment/stripe.expand';
        // }
        
        $template_file = 'default/template/payment/stripe.expand';
		...
		if (version_compare(VERSION, '2.0', '<')) {
			...
			return $this->load->view($template_file, $this->data);
		}
	}
	...
	public function chargeToken() {
		...
		$language_data = $this->language->load_json(version_compare(VERSION, '2.3', '<') ? 'total/total' : 'extension/total/total');
		...
		if (!empty($this->request->post['embed'])) {
			...
			if ($settings['checkout_shipping']) {
				...
				if ($shipping_quote['country_id'] != $shipping_country->row['country_id'] || 
					...
					echo $settings['error_shipping_mismatch_' . $language];
					return;
				}
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				if (!empty($settings['subscription_options'])) {
					foreach ($settings['subscription_options'] as $row) {
						foreach ($product['option'] as $option) {
							...
							if ($option['name'] == $row['option_name'] && $option_value == $row['option_value']) {
				...
				if (!empty($product['recurring']) && !empty($settings['subscription_profiles'])) {
					foreach ($settings['subscription_profiles'] as $row) {
						$profile_name = (version_compare(VERSION, '2.0', '<')) ? $product['profile_name'] : $product['recurring']['name'];
						if ($profile_name == $row['profile_name']) {
							$plan_ids[] = trim($row['plan_id']);
						}
					}
				}
				...
				foreach ($plan_ids as $plan_id) {
					...
					if (!empty($plan_response['error'])) {
						...
						echo $settings['error_customer_required_' . $language];
						return;
					}
			...
			foreach ($plans as &$plan) {
				...
				if ($plan['trial']) {
					...
					if (!empty($subscription_response['error'])) {
						echo $subscription_response['error']['message'];
						return;
					}
				...
				if (isset($this->session->data['shipping_method']) && $stripe_customer_id && !empty($settings['include_shipping'])) {
							...
							foreach ($quote['quote'] as $q) {
								if ($q['code'] == $order_info['shipping_code']) {
									...
									if (!empty($invoice_item_response['error'])) {
										echo $invoice_item_response['error']['message'];
										return;
									}
					...
					foreach ($cart_products as $product) {
						...
						if (version_compare(VERSION, '1.5.6', '<')) {
							...
							$recurring_profile = $product['recurring']['recurring_id'];
						}
				...
				if (!empty($plan['trial'])) {
					...
					if (version_compare(VERSION, '2.0', '<')) {
						$this->db->query("UPDATE " . DB_PREFIX . "order_total SET text = '" . $this->db->escape($this->currency->format($order_info['total'], $order_info[isset($order_info['currency']) ? 'currency' : 'currency_code'])) . "', value = " . (float)$order_info['total'] . " WHERE order_id = " . (int)$order_info['order_id'] . " AND title = '" . $this->db->escape($language_data['text_total']) . "'");
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = " . (int)$order_info['order_id'] . ", code = 'total', title = '" . $this->db->escape($settings['text_to_be_charged_' . $language] . ' (' . $plan['name'] . ')') . "', text = '" . $this->db->escape($this->currency->format(-$total_plan_cost, $order_info[isset($order_info['currency']) ? 'currency' : 'currency_code'])) . "', value = " . (float)-$total_plan_cost . ", sort_order = " . ((int)$this->config->get('total_sort_order')-1));
					} else {
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = " . (int)$order_info['order_id'] . ", code = 'total', title = '" . $this->db->escape($settings['text_to_be_charged_' . $language] . ' (' . $plan['name'] . ')') . "', value = " . (float)-$total_plan_cost . ", sort_order = " . ((int)$this->config->get('total_sort_order')-1));
					}
				}
			}
		}
		...
		if (!empty($charge_response)) {
			...
			$flag_image = (version_compare(VERSION, '2.3', '<')) ? ' &nbsp; <img src="../image/flags/' . strtolower($charge_response['source']['country']) . '.png" />' : '';
			
			// Apple Pay fields
			if (!empty($charge_response['source']['tokenization_method']))	$comment .= $strong . 'Payment Method:</strong>' . ucwords(str_replace('_', ' ', $charge_response['source']['tokenization_method'])) . '<br />';
			if (!empty($charge_response['source']['dynamic_last4']))		$comment .= $strong . 'Device Number:</strong>>**** **** **** ' . $charge_response['source']['dynamic_last4'] . '<br />';
			...
			if (isset($charge_response['source']['country']))				$comment .= $strong . 'Origin:</strong>' . $charge_response['source']['country'] . $flag_image . '<br />';
			...
			$comment .= $strong . 'Refund:</strong><a onclick="refund($(this), ' . number_format($charge_response['amount'] / 100, 2, '.', '') . ', \'' . $charge_response['id'] . '\', ' . (version_compare(VERSION, '1.5.0', '<') ? 0 : 1) . ')">(Refund)</a>';
		}
	...
	public function completeOrder() {
		...
		$this->load->model('checkout/order');
		$this->model_checkout_order->{version_compare(VERSION, '2.0', '<') ? 'confirm' : 'addOrderHistory'}($order_id, $order_status_id);
	}
	...
	public function webhook() {
		...
		if ($event->type == 'charge.refunded') {
			...
			*/
			...
			// Set order totals
			$decimal_factor = (in_array(strtoupper($event->data->object->currency), array('BIF','CLP','DJF','GNF','JPY','KMF','KRW','MGA','PYG','RWF','VND','VUV','XAF','XOF','XPF'))) ? 1 : 100;
	...
	private function curlRequest($request, $api, $data = array()) {
		...
		//Check if there is decline code from stripe
