...
class ControllerPaymentOneCard extends Controller {
	...
	protected function index() {
		...
		$this->data = array_merge($this->data, $this->language->load_json('payment/' . $classname));
        ...
        // if(file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/' . $classname . '.expand')) {
        //     $this->template = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/' . $classname . '.expand';
        // }
        // else {
        //     $this->template = $this->config->get('config_template') . '/template/payment/' . $classname . '.expand';
        // }
        
        $this->template = 'default/template/payment/' . $classname . '.expand';
	...
	public function callback() {
		...
		if ($_REQUEST['OneCard_Code'] == '00' || $_REQUEST['OneCard_Code'] == '18') {
        	...
        	if ($my_downloadmd5 == $_REQUEST['OneCard_RHashKey']) {
				...
				mail((!empty($this->config->get('config_smtp_username')) ? $this->config->get('config_smtp_username') :  $this->config->get('config_email')), 'ATTN: Unverified Order', "Order ID: $order_id needs manual review. \r\n\r\n $my_downloadmd5 != " . $_REQUEST['OneCard_RHashKey']);
			}
