...
class ControllerPaymentIyzicoCheckoutForm extends Controller {
        ...
        //here we check if we are in test mode or not to set end point to send request to
        
        $testMode = $this->config->get('iyzico_checkout_form_test_mode');
        $this->base_url = ($testMode == 1) ? "https://sandbox-api.iyzipay.com" : "https://api.iyzipay.com";
    }
    ...
    private $order_prefix = "opencart23_";
        ...
        public function index() {
                ...
                $this->data = array_merge($this->data, $this->language->load_json('payment/iyzico_checkout_form'));
                ...
                if(isset($this->session->data['error_iyzico']) && $this->session->data['error_iyzico']['resp_code'])
                    $this->data['iyzico_error'] = $this->session->data['error_iyzico']['resp_code'].": ".
                $this->session->data['error_iyzico']['resp_msg'];  
                
                $this->template = 'default/template/payment/iyzico_checkout_form.expand';
        ...
        public function gettoken() {
                ...
                try {
                        ...
                        if(!in_array($order_currency, ['TRY', 'USD', 'EUR', 'GBP', 'IRR'])){
                            $order_currency = 'TRY';
                            $iyzico_total_amount = number_format(
                                                        round(
                                                            $this->currency->convert(
                                                                $cart_total_amount, $this->currency->getCode(), 'TRY'
                                                            ), 2
                                                        ), 2, '.', ''
                                                    );
                        }else{
                            $iyzico_total_amount = $cart_total_amount;
                        }
                        ...
                        $request->setPaidPrice($iyzico_total_amount);
                        ...
                        $request->setCurrency($order_currency);
        ...
        public function callback() {
                ...
                try {
                        ...
                        if ($response->getStatus() == "failure") {
                                $this->session->data['error_iyzico']['resp_code'] = $response->getErrorCode();
                                $this->session->data['error_iyzico']['resp_msg'] = $response->getErrorMessage();
                                $this->response->redirect($this->url->link('checkout/checkout', '', $server_conn_slug));
                        }

                        if ($response->getPaymentStatus() == "FAILURE") {
                                $this->session->data['error_iyzico']['resp_code'] = $response->getErrorCode();
                                $this->session->data['error_iyzico']['resp_msg'] = 'Payment FAILURE';
                                $this->response->redirect($this->url->link('checkout/checkout', '', $server_conn_slug));
                        }
                       ...
                       // $this->model_payment_iyzico_checkout_form->updateOrderEntry($update_data_array, $get_auth_table_id);
                        ...
                        if (empty($response_order_id) || $response_order_id != $order_id) {
                                $this->session->data['error_iyzico']['resp_code'] = '';
                                $this->session->data['error_iyzico']['resp_msg'] = 'Invalid Order';
                                $this->response->redirect($this->url->link('checkout/checkout', '', $server_conn_slug));
                        }
                        ...
                        if (!empty($order_info['order_status_id']) && $order_info['order_status'] != null){
                                $this->session->data['error_iyzico']['resp_code'] = '';
                                $this->session->data['error_iyzico']['resp_msg'] = 'order_already_exists';
                                $this->response->redirect($this->url->link('checkout/checkout', '', $server_conn_slug));
                        }
                        ...
                        $this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('iyzico_checkout_form_order_status_id'));
                        ...
                        $this->session->data['error_iyzico']['resp_code'] = '';
                                $this->session->data['error_iyzico']['resp_msg'] = $resp_msg;
                        $this->response->redirect($this->url->link('checkout/checkout', '', $server_conn_slug));
                }
        ...
        public function error() {
                ...
                if (!empty($error['response']['state']) && $error['response']['state'] == 'failed') {
                        ...
                        $this->data['iyzico_error'] = $error['response']['error_message'];
                }
