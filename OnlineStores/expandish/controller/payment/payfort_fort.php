...
class ControllerPaymentPayfortFort extends Controller
{
    ...
    public function index()
    {
        ...
        $template = 'payfort_fort.expand';
        if ($this->pfConfig->isCcMerchantPage()) {
            $template                             = 'payfort_fort_merchant_page.expand';
        ...
        elseif ($this->pfConfig->isCcMerchantPage2()) {
            $template                             = 'payfort_fort_merchant_page2.expand';
            $this->data['payment_request_params'] = $this->pfPayment->getPaymentRequestParams($this->paymentMethod, $this->integrationType);
        ...
        // if(file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/' . $template)) {
        //     $this->template = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/' . $template;
        // }
        // else {
    ...
    private function _handleResponse($response_mode = 'online', $integration_type = PAYFORT_FORT_INTEGRATION_TYPE_REDIRECTION)
    {
        ...
        if (!is_array($success) && $success) {
            ...
            $this->model_checkout_order->confirm(
                ...
                $this->config->get('payfort_fort_order_status_id')
            );
        }
        else {
            $this->session->data['payfort_msg_error'] = $success[1];
            $redirectUrl = 'payment/payfort_fort/error';
        }
        if ($this->pfConfig->isCcMerchantPage()) {
    ...
    public function error()
    {
        ...
        $this->data['continue'] = $this->url->link('common/home');
        $this->data['response_error_text_message'] = $this->session->data['payfort_msg_error']?:'';
