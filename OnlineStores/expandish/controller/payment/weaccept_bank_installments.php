...
/**
 * Store front for WeAccept Bank Installments

 */
class ControllerPaymentWeAcceptBankInstallments extends Controller
{
	...
	{
			// if (file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/weaccept_bank_installments.expand')) {
			// 	$this->template = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/weaccept_bank_installments.expand';
			// } else {
			// 	$this->template = 'default/template/payment/weaccept_bank_installments.expand';
			// }
            $this->template = 'default/template/payment/weaccept_bank_installments.expand';
			$this->render_ecwig();
    ...
    public function confirm_payment()
    {
            ...
            $this->load->model('payment/weaccept_bank_installments');
            $this->language->load_json('payment/weaccept_bank_installments');

            //Step 1: Get a token 
            $token = $this->model_payment_weaccept_bank_installments->get_auth_token($this->config->get('weaccept_online_api_key'))->token;
            //Get integration id
            $integration_id = $this->config->get('weaccept_bank_installments_integration_id');
            //Get merchant id
            $merchant = $this->config->get('weaccept_online_merchant_id');
            //Get bank_installments iframe id
            $iframe_id = $this->config->get('weaccept_bank_installments_iframe_id');
            //Get order info
            ...
            $weaccept_order = $this->model_payment_weaccept_bank_installments->get_weaccept_order($order['order_id']);
            if (!$weaccept_order) {
                $merchant_order_id = $order['order_id'] . '.' . time();

                $order_registration_request = $this->model_payment_weaccept_bank_installments->get_order_registeration_request($token, $merchant, $merchant_order_id, (int)$total_egp * 100, 'EGP', $shipping_address);

                if (!$order_registration_request) {
                    die('Unable to register the order to Accept servers.<br>' . $this->model_payment_weaccept_bank_installments->getLastError());
                    exit();
                }
                ...
                //insert into DB
                $this->model_payment_weaccept_bank_installments->insertWeAcceptOrder($customerId, $order, $weaccept_order, $merchant_order_id, $order_registration_request);
            ...
            $payment_request = $this->model_payment_weaccept_bank_installments->get_payment_key_request($token, (int)$total_egp * 100, $weaccept_order[0]['weaccept_order_id'], 'EGP', $integration_id, $billing_address);
            if (!$payment_request) {
                $this->data['error_payment'] = $this->language->get('error_unable_to_register_payment_key');
                die($this->model_payment_weaccept_bank_installments->getLastError());
            }
	...
	private function callback()
	{
		...
		$hash = $this->model_payment_weaccept_bank_installments->calculateHash($hmac, $data, $type);
			...
			if ($post) {
				...
				if ($type == 'TRANSACTION') {
					...
					if ($data['success'] == "true") {
						// 5 = completed
						$this->model_checkout_order->confirm($order_id,$this->config->get('weaccept_bank_installments_completed_order_status_id'), 'Payment Sucessfull.');
					} else {
						// 10 = fail
						$this->model_checkout_order->confirm($order_id, $this->config->get('weaccept_bank_installments_failed_order_status_id'), 'Payment Declined.');
					}
				}
			} else {
				if ($data['success'] == 'true') {
