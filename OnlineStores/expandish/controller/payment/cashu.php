...
class ControllerPaymentCashu extends Controller {
	...
	protected function index() {
		...
		//to view products name in payment page
        $products=$this->model_checkout_order->getOrderProducts($this->session->data['order_id']);
		$products_name = implode(' / ', array_column($products, 'name'));
		...
		$this->data = array_merge($this->data, $this->language->load_json('payment/' . $this->name));
		...
		$this->data['fields']['display_text'] =$products_name;
			//($this->config->get('config_name')) ? $this->config->get('config_name') : $this->config->get('config_store');
        ...
        // }
        $this->template = 'default/template/payment/cashu.expand';
		$this->render_ecwig();
	}

	public function confirm() {

		$order_status_id = $this->config->get('cashu_order_status_id') ? $this->config->get('cashu_order_status_id') : $this->config->get('config_order_status_id');
		$this->load->model('checkout/order');
	...
	public function callback() {
		...
		if ($this->config->get($this->name . '_debug')) {
			...
			$msg = ($p_msg . "\r\n" . $g_msg);
			mail((!empty($this->config->get('config_smtp_username')) ? $this->config->get('config_smtp_username') :  $this->config->get('config_email')), $this->name . '_debug', $msg);
