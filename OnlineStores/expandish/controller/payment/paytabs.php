...
class ControllerPaymentPaytabs extends PaymentController
{
    ...
    public function __construct($registry)
    {
        ...
        $this->data['is_external'] = $this->data['is_external'] && $this->config->get('paytabs_checkout_options') === 'redirect_to_payment_gateway'; 
    }
...
// class constructor
    ...
    public function index()
    {
        ...
        $this->data['action'] = "index.php?route=payment/paytabs/send";
        ...
        // if (file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/paytabs.expand')) {
        //     $this->template = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/payment/paytabs.expand';
        // } else {
        //     $this->template = $this->config->get('config_template') . '/template/payment/paytabs.expand';
        // }
        $this->template = 'default/template/payment/paytabs.expand';
        if($this->data['paytabs_checkout_options'] == 'pay_on_our_site'){
        //     $this->template = $this->config->get('config_template') . '/template/payment/paytabs_popup.expand';
             $this->data['payment_data'] = $this->send();
        }
        $this->render_ecwig();
    }
    ...
    public function callback()
    {
        ...
        //if (isset($_SESSION['secret_key'])) {
            ...
            $paytabs_merchant = $this->config->get('paytabs_merchant');

            $request_param = array('secret_key' => $secret_key, 'merchant_email' => $paytabs_merchant, 'payment_reference' => $this->request->post['payment_reference']);
            ...
            $paytabsResponseCodeRange = [100, 481, 482, 483];
            ...
            if (in_array($response, $paytabsResponseCodeRange)) {

                $this->paymentTransaction->insert([
                    'order_id' => $object->reference_no,
                    'transaction_id' => $this->request->post['transaction_id'],
                    'payment_gateway_id' => $this->paymentMethod->selectByCode('paytabs')['id'],
                    'payment_method' => '',
                    'status' => 'Success',
                    'amount' => '',
                    'currency' => '',
                    'details' => json_encode($this->request->post),
                ]);

                $this->model_checkout_order->confirm( $object->reference_no , $this->config->get('paytabs_completed_order_status_id'), '', false);
                $this->log('info','Payment Verified Successfully for order_ID :- ' . $this->session->data['order_id'] . ', order_Status : ' . $this->config->get('paytabs_completed_order_status_id') . ', response_data : ' . $response_data);
                ...
                $this->log('error','Payment Verified failed for order_ID :- ' . $this->session->data['order_id'] . ', order_Status : ' . $this->config->get('paytabs_failed_order_status_id') . ', payment_rejected_comment : ' . $this->language->get('payment_rejected_comment') .', response_data : ' . $response_data );
    ...
    public function send()
    {
         ...
         */
        $telephone = str_replace($this->getccPhone($order_info['payment_iso_code_2']),'',$order_info['telephone']);
       ...
       if($order_info['shipping_city']){$shipping_city= $order_info['shipping_city'];}else{$shipping_city= $order_info['payment_city'];}
        ...
        $payment_data = array(
            ...
            'merchant_id' => $this->config->get('paytabs_merchant_id'),
            'secret_key' => $this->config->get('paytabs_security'),
            ...
            'title' => 'Order #'.$this->session->data['order_id'].', Bill To: '.$order_info['payment_firstname'] . ' ' . $order_info['payment_lastname'],
            'ip_customer' => $_SERVER['REMOTE_ADDR'],
            'ip_merchant' => $_SERVER['SERVER_ADDR'],
            'address_shipping' =>$address_shipping,
            'city_shipping' => $shipping_city,
            'state' => $order_info['payment_zone'],
            'state_shipping' => $shipping_zone,
...
//echo('<pre>'); print_r($payment_data); die;
        if($this->data['paytabs_checkout_options'] == 'redirect_to_payment_gateway'){
            ...
            $this->log('info','bitpay request with order_Data : ' . var_export($order_info,true) . ', response ' . $object->result);
        ...
        }else if($this->data['paytabs_checkout_options'] == 'pay_on_our_site'){
            return $payment_data;
        }
