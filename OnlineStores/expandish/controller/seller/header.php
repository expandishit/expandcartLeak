<?php
class ControllerSellerHeader extends Controller {
	...
	protected function index() {
        ...
        if ($rss=="latest" || $rss=="featured" || $rss=="special" || $rss=="bestseller") {
            ...
            foreach ($products as $product) {
                ...
                $price = ($isCustomerAllowedToViewPrice) ? $this->currency->format($this->tax->calculate($product["price"], $product["tax_class_id"], $this->config->get("config_tax"))) : false;
                $special = ( (float)$product["special"] && $isCustomerAllowedToViewPrice) ? $this->currency->format($this->tax->calculate($product["special"], $product["tax_class_id"], $this->config->get("config_tax"))) : false;
		...
		$this->data['inlineScripts'] = $this->document->getInlineScripts();
		$this->data['lang'] = $this->language->get('code');
		$this->data['direction'] = $this->language->get('direction');
		$this->data['google_analytics'] = html_entity_decode($this->config->get('config_google_analytics'), ENT_QUOTES, 'UTF-8');
        $this->data['name'] = $this->config->get('config_name');
        $this->data['is_responsive_enabled'] = $this->config->get('msconf_responsive');
		
		if ($this->config->get('config_icon')) {
            // $this->data['icon'] = $server . 'image/' . STORECODE . '/' . $this->config->get('config_icon');
			$this->data['icon'] = \Filesystem::getUrl('image/' . $this->config->get('config_icon'));
		} else {
			$this->data['icon'] = '';
		}
		...
		$this->template = 'default/template/multiseller/header.tpl';
