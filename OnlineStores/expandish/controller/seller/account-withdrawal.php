...
class ControllerSellerAccountWithdrawal extends ControllerSellerAccount {
	public function jxRequestMoney() {
		...
		$json = array();
		$this->data['msconf_payment_method'] = $this->config->get('msconf_payment_method');
		if (!$this->MsLoader->MsSeller->getPaypal() && $this->data['msconf_payment_method'] == 1) {
	...
	public function index() {
		...
		$this->document->addScript('expandish/view/javascript/multimerch/account-withdrawal.js');
		...
		$this->data['msconf_payment_method'] = $this->config->get('msconf_payment_method');
		if($this->data['msconf_payment_method'] == 1){
