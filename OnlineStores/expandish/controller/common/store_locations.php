...
class ControllerCommonStoreLocations extends Controller {
	...
	public function index() {
		...
		if ( $this->model_module_store_locations->isModuleInstalled() != true )
		{
			$this->redirect('common/home');
		}

		$this->language->load_json('common/store_locations');
		...
		if (file_exists('expandish/view/theme/' . $this->config->get('config_template') . '/stylesheet/gmap-style.css')) {
			$this->document->addStyle('expandish/view/theme/' . $this->config->get('config_template') . '/stylesheet/gmap-style.css');
		} else {
			$this->document->addStyle('expandish/view/theme/default/css/gmap-style.css');
		}
		...
		$MAP_OBJECT->api_key = $this->config->get('store_locations_api_key');
		...
		$counter = 0;
		foreach($Locations as $Location) {
			$Location['Details'] = substr(html_entity_decode($Location['Details']), 0, 80);
		...
		//Create a json object for use in search
		$this->data['locations_json'] = json_encode($this->data['Locations']);

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/store_locations.expand')) {
			$this->template = $this->config->get('config_template') . '/template/common/store_locations.expand';
		} else {
			$this->template = 'default/template/common/store_locations.expand';
		}
