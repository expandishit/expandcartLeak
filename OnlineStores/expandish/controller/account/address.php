...
class ControllerAccountAddress extends Controller {
    ...
    public function index() {
    	...
    	$this->language->load_json('account/address');
  	...
  	public function insert() {
    	...
    	$this->language->load_json('account/address', $this->identityAllowed());
  	...
  	protected function getList() {
        ...
        if(file_exists(DIR_TEMPLATE . 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/account/address_list.expand')) {
            $this->template = 'customtemplates/' . STORECODE . '/' . $this->config->get('config_template') . '/template/account/address_list.expand';
        }
        else {
            $this->template = $this->config->get('config_template') . '/template/account/address_list.expand';
        }
  	...
  	protected function getForm() {
		...
		$modLocals = $this->model_account_signup->gatLocals($lang_id);
		...
		if($isActive1){
			$firstname=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'f_name_'.$this->config->get('config_language_id'))['value'];
			$lastname=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'l_name_'.$this->config->get('config_language_id'))['value'];
			$telephone=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'telephone_'.$this->config->get('config_language_id'))['value'];
			$fax=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'fax_'.$this->config->get('config_language_id'))['value'];
			$company=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'company_'.$this->config->get('config_language_id'))['value'];
			$companyid=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'companyid_'.$this->config->get('config_language_id'))['value'];
			$address_1=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'address1_'.$this->config->get('config_language_id'))['value'];
			$address_2=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'address2_'.$this->config->get('config_language_id'))['value'];
			$postcode=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'pin_'.$this->config->get('config_language_id'))['value'];
			$city=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'city_'.$this->config->get('config_language_id'))['value'];		
			$country=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'country_'.$this->config->get('config_language_id'))['value'];
			$zone=$this->model_account_signup->getModLocal($this->config->get('config_language_id'),'state_'.$this->config->get('config_language_id'))['value'];
		
		}
		
		$this->data['entry_firstname'] =$firstname!=""?$firstname:$this->language->get('entry_firstname');
		$this->data['entry_lastname'] = $lastname!=""?$lastname:$this->language->get('entry_lastname');
		$this->data['entry_telephone'] = $telephone!=""?$telephone:$this->language->get('entry_telephone');
		$this->data['entry_fax'] = $fax!=""?$fax:$this->language->get('entry_fax');
		$this->data['entry_company'] = $company!=""?$company:$this->language->get('entry_company');
		$this->data['entry_company_id'] = $companyid!=""?$companyid:$this->language->get('entry_company_id');
        $this->data['entry_address_1'] = $address_1!=""?$address_1:$this->language->get('entry_address_1');
		$this->data['entry_address_2'] = $address_2!=""?$address_2:$this->language->get('entry_address_2');
		$this->data['entry_postcode'] = $postcode!=""?$postcode:$this->language->get('entry_postcode');
		$this->data['entry_city'] = $city!=""?$city:$this->language->get('entry_city');
		$this->data['entry_country'] = $country!=""?$country:$this->language->get('entry_country');
		$this->data['entry_zone'] = $zone!=""?$zone:$this->language->get('entry_zone');
		$this->data['entry_area'] = $area!=""?$area:$this->language->get('entry_area');
	...
	if (isset($this->request->post['area_id'])) {
			...
			$this->data['area_id'] = $address_info['area_id'];
	  } else {
			$this->data['area_id'] = '';
	  }
        ...
        $template_file_name = 'address_form.expand';

        if ($isActive1)
            $template_file_name = 'myaddress_form.expand';
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/' . $template_file_name)) {
			...
			$this->template = 'clearion/template/account/' . $template_file_name;
		}
  	...
  	protected function validateForm() {
		...
		$signupIsActive = $this->model_account_signup->isActiveMod()['enablemod'] ?? 0;
		$signupModuleData = $signupIsActive ? $this->model_account_signup->getModData() : [];

		if ((utf8_strlen($this->request->post['firstname']) < 1) || (utf8_strlen($this->request->post['firstname']) > 32)) {
			if ($signupIsActive) {
				if ($signupModuleData['f_name_req'] == 1) {
		...
		if ((utf8_strlen($this->request->post['lastname']) < 1) || (utf8_strlen($this->request->post['lastname']) > 32)) {
			if ($signupIsActive) {
				if ($signupModuleData['l_name_req'] == 1) {
		...
		if ((utf8_strlen($this->request->post['address_1']) < 3) || (utf8_strlen($this->request->post['address_1']) > 128)) {
			if ($signupIsActive) {
				if ($signupModuleData['address1_req'] == 1) {
		...
		if ((utf8_strlen($this->request->post['city']) < 2) || (utf8_strlen($this->request->post['city']) > 128)) {
			if ($signupIsActive) {
				if ($signupModuleData['city_req'] == 1) {
		...
		if (\Extension::isInstalled('seller_based') && $this->config->get('seller_based_status') == 1){
			if (!isset($this->request->post['area_id']) || $this->request->post['area_id'] == '' || !is_numeric($this->request->post['area_id'])) {
	...
	public function country() {
		...
		if ($country_info) {
			...
			$json = array(
				...
				'zone'              => $this->model_localisation_zone->getZonesByCountryId((int)$this->request->get['country_id']),
    ...
    protected function renderContent()
    {
        ...
        foreach ($results as $result) {
            ...
            $this->data['addresses'][] = array_merge($result, array(
		...
		$this->data['include_page'] = 'address_list.expand';
    ...
    private function loadTaps($active_tap)
    {
        $this->data['taps'] = $this->getChild('account/taps/renderTaps', $active_tap);
    }
    ...
    private function _checkIfCustomerHasEditPermission(){
        ...
        if (\Extension::isInstalled('customer_profile_permissions') && 
        	$settings['status'] == 1 &&
        	$settings['limit_cust_edit_address_data'] == 1 &&
        	in_array($this->customer->getCustomerGroupId(), $settings['limit_cust_edit_address_data_groups']) ){
        	$this->session->data['warning'] = $this->language->get('editing_address_error');
