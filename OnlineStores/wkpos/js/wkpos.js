...
function loginUser() {
  ...
  if (validate_login == true) {
    ...
    $.ajax({
      url: 'wkpos/index.php?route=wkpos/wkpos/userLogin',
      dataType: 'json',
      type: 'post',
      data: {
        username: username,
...
function printProducts() {
  ...
  for (var i = 0; i < product_count; i++) {
    if (pos_products[i]) {
      ...
      html = '<div class="col-md-3 col-sm-4 col-xs-6 product-select" product-id="' + pos_products[i]["product_id"] + '" option="' + pos_products[i]["option"] + '">';

      html += '  <img src="' + pos_products[i]["image"] + '" class="product-image" width="100%" height="100%">';
      html += '  <div class="col-xs-12 product-detail">';
      html += '    <b class="product-title">' + pos_products[i]["name"] + '</b>';
      if (pos_products[i]["special"] == 0) {
        html += entry_price + ' <b class="product-price">' + pos_products[i]["price"] + '</b>';
      } else {
...
function showExpenses(json,currency_code) {
//expenses-table

  let expensesTable = $('.expenses-table');
  expensesTable.html('');
  if (json['expenses'].length < 1){
    expensesTable.html(`<H6>${error_expenses}<H6/>`);
  }else {
    let html = `<div class="col-sm-12" id="expenses-container" style="height: 176px;">
          <div class="row" id="others"><div class="table-responsive">  <table class="table table-bordered table-hover"> 
             <thead class="btn-info">  
                 <tr>   
                  <td>${text_id}</td>       
                   <td>${text_date}</td>       
                   <td class="text-center">${text_expenses_title}</td>    
                  <td>${text_expenses_desc}</td>    
                  <td>${text_expenses_amount}</td>    
    ...
    for (var i = 0; i < json['expenses'].length; i++) {
      console.log(json['expenses'].length);
      html +=`
                <tr>   
                  <td> ${i + 1}</td>       
                   <td>${expenses[i]['date_added']}</td>       
                   <td class="text-center">${expenses[i]['title']}</td>    
...
function getPopularProducts() {
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/product/getPopularProducts',
    dataType: 'json',
    type: 'get',
    beforeSend: function () {
...
function getAllProducts() {
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/product&start=' + start,
    dataType: 'json',
    type: 'post',
    data: {
      user_id: user_login
    ...
    success: function (json) {
      ...
      $.each(json['products'], function (key, value) {
        ...
        if (typeof value.is_booking != 'undefined' && value.is_booking > 0) {
          booking_products.push(parseInt(key));
        }
      });
...
function getAllCategories() {
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/category',
    dataType: 'json',
    type: 'post',
    data: {
      user_id: user_login
...
function getAllCustomers(nocontinue) {
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/customer',
    dataType: 'json',
    type: 'get',
    beforeSend: function () {
...
function getAllOrders() {
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/order',
    dataType: 'json',
    type: 'post',
    data: {
      user_id: user_login
...
function getAllReturns() {
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/return',
    type: 'post',
    dataType: 'json',
    data: {
      user_id: user_login
  ...
  if (search_check) {
    ...
    for (var i = 0; i < product_count; i++) {
      if (pos_products[i]) {
        ...
        if (pos_products[i]["name"].toLowerCase().search(search_keyword) != '-1' || pos_products[i]["model"].toLowerCase().search(search_keyword) != '-1' || pos_products[i]["sku"].toLowerCase().search(search_keyword) != '-1' || pos_products[i]["barcode"].toLowerCase().search(search_keyword) != '-1') {
          html = '<div class="col-md-3 col-sm-4 col-xs-6 product-select" product-id="' + pos_products[i]["product_id"] + '" option="' + pos_products[i]["option"] + '">';
          ...
          html += '    <b class="product-title">' + pos_products[i]["name"] + '</b>';
          if (pos_products[i]["special"] == 0) {
            html += '<br>' + entry_price + ' <b class="product-price">' + pos_products[i]["price"] + '</b>';
          } else {
            html += '<br>' + entry_price + ' <b>' + pos_products[i]["special"] + '</b> <span class="line-through">' + pos_products[i]["price"] + '</span>';
          };
          html += '  </div>';
          if (pos_products[i]['option']) {
            html += '<span class="label label-info option-noti" data-toggle="tooltip" title="' + text_option_notifier + '"><i class="fa fa-question-circle"></i></span>';
...
function onlineStatus($toast) {
  ...
  if (status_button.hasClass('btn-danger')) {
    if (navigator.onLine) {
      if ($toast) {
        $.toaster({
          priority: 'success',
      ...
      status_button.removeClass('btn-danger').addClass('btn-success').html('<span class="btn-label"><i class="fa fa-toggle-on"></i></span> <span class="hidden-xs">' + text_online + '</span>');
      offline = 0;
    } else {
      if ($toast) {
        $.toaster({
    ...
    status_button.removeClass('btn-success').addClass('btn-danger').html('<span class="btn-label"><i class="fa fa-toggle-off"></i></span> <span class="hidden-xs">' + text_offline + '</span>');
    offline = 1;
  }
}
...
$(document).on('click', '.wkpaymentmethod', function () {
  ...
  if (type == 'cash-payment') {
    ...
    $('.accept-payment').attr('ptype', type);
  };
});
...
function acceptPayment(thisthis) {
  ...
  if (!offline) {
    ...
    $.ajax({
      ...
      dataType: 'json',
      data: {
        cart: pos_cart[current_cart],
        payment_method: payment_type,
        partial_credit_payment: partial_credit_payment,
        only_credit_payment: only_credit_payment,
        shipping_method: shipping_method,
        shipping_charge: delivery_charge,
        use_credit: use_credit,
        credit: credit_amount,
        add_credit: global_change,
      ...
      success: function (json) {
        ...
        if(json['order_id']){
          var invoice2_ulr = $('#normal-invoice2').attr('data-href') + json['order_id'];
          $('#normal-invoice2').attr('href', invoice2_ulr);
        }
...
$(document).on('click', '#button-expense', function () {
  $('.cash-payment').addClass('hide');
  if ($('.expense-parent').hasClass('panel-show')) {
    $(this).removeClass('onfocus');
    $('.expense-parent').removeClass('panel-show');
    $('.sidepanel').removeClass('sidepanel-show');
    return;
  };
  $('#sidepanel-inner').css('display', 'none');
  ...
  $('.wksidepanel').removeClass('onfocus');
  $(this).addClass('onfocus');
  $('.expense-parent').addClass('panel-show');
  $('.sidepanel').removeClass('sidepanel-show');
  if ($('#expenses').text() == '') {
    $('.other-child .wkexpense:first').trigger('click');
  }
});
...
$(document).on('click', '.wkorder', function () {
  ...
  if (otype == 1) {
    for (var i = 0; i < Object.keys(orders).length; i++) {
      order_html += '<div class="col-sm-3 order-display cursor" order-id="' + orders[i]['order_id'] + '">';
      ...
      order_html += '  <div class="table-responsive table-order order-total-table">';
...
$(document).on('click', '.wkother', function () {
  ...
  if (otype == 1) {
    ...
    for (var i = 0; i < product_count; i++) {
      if (pos_products[i]) {
        if (parseInt(pos_products[i]['quantity']) <= low_stock) {
          html = '<div class="col-md-3 col-sm-4 col-xs-6 product-select" product-id="' + pos_products[i]["product_id"] + '">';
    ...
    request_html += '        <td>' + entry_product_name + '</td>';
    request_html += '        <td style="width: 150px;">' + column_quantity + '</td>';
    request_html += '        <td>' + text_supplier + '</td>';
    request_html += '        <td>' + text_comment + '</td>';
    request_html += '        <td>' + text_supplier + '</td>';
    request_html += '      </tr>';
    request_html += '    </thead>';
    request_html += '    <tbody>';
    for (var i = 0; i < product_count; i++) {
      if (pos_products[i]) {
        if (parseInt(pos_products[i]['quantity']) <= low_stock) {
          ...
          request_html += '  <td><textarea class="form-control request-comment" placeholder="' + text_comment + '"></textarea></td>';
          if (pos_products[i]['requests'] == 0) {
            request_html += '  <td>' + text_no_requests + '</td>';
          } else {
    ...
    request_html += '    <thead class="btn-info">';
    request_html += '      <tr>';
    request_html += '        <td> ' + text_order_id + '</td>';
    request_html += '        <td>' + text_date + '</td>';
    request_html += '        <td class="text-center">' + text_order_details + '</td>';
    request_html += '        <td>' + entry_status + '</td>';
    request_html += '      </tr>';
    request_html += '    </thead>';
    request_html += '    <tbody>';
...
$(document).on('click', '.wkexpense', function () {
  var expense_panel = $('#expenses');
  $('.wkexpense').removeClass('onfocus');
  var otype = $(this).addClass('onfocus').attr('otype');
  //expense_panel.html('');
  var expense_html = '';
  if (otype == 1) {
...
$(document).on('click', '.order-display', function () {
  ...
  if (order_id) {
    ...
    var invoice_ulr = $('#normal-invoice').attr('data-href') + order_id;
    $('#normal-invoice').attr('href', invoice_ulr);
    ...
    for (var i = 0; i < Object.keys(order_products[order_id]['products']).length; i++) {
      ...
      order_html += '<tr>';
      order_html += '  <td class="text-left">' + order_products[order_id]['products'][i]['name'] + '<ul style="list-style: none; padding: 0;">      <li><span class="label label-warning" style="white-space: normal;">' + option_value + '</span></li>    </ul></td>';
      ...
      order_html += '<tr>';
      print_order_html += '<tr>';
      print_order_html += '  <td style="text-align: left">' + order_products[order_id]['products'][i]['name'] + '<span class="label label-warning" style="white-space: normal;font-style: italic;">' + option_value_print + '</span></td>';
    ...
    if (order_products[order_id]['qr_code']){
      let qr_code_html= '<img src="'+order_products[order_id]['qr_code'] + '" />';
      qr_code_html += '<br>';
      qr_code_html += 'Scan By ZATCA App';
...
$(document).on('click', '.button-accounts', function () {
  ...
  if (stype == 'basic') {
    ...
    $.ajax({
      url: 'wkpos/index.php?route=wkpos/wkpos/updateProfile',
      dataType: 'json',
      type: 'post',
      data: postDetails,
      beforeSend: function () {
  ...
  if (stype == 'other') {
    ...
    $.ajax({
      url: 'wkpos/index.php?route=wkpos/wkpos/changeSettings',
      dataType: 'json',
      type: 'post',
      data: {
        language: language,
      ...
      success: function (json) {
        if (json['success']) {
          ...
          location = 'wkpos/index.php?route=wkpos/wkpos';
          $('#loader').css('display', 'block');
...
$(document).on('click','.button-expenses',function (e) {
  e.preventDefault();
  let title = $("#title").val();
  let description = $("#description").val();
  let amount = $("#amount").val();
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/expense/addExpense',
    dataType: 'json',
    type: 'post',
    data: {
      title: title,
    ...
    success: function (json) {
      if (json['success']) {
        alert(text_expense_success_message);
        $("#expenses-form").trigger('reset');
        getAllExpenses();
      };

      if (json['error']) {
        if (json['title'] && !$("#title").parent().parent().hasClass("has-error") ) {

          $('#title')
              .after('<div class="text-danger">' + json['title'] + '</div>').parent().parent()
              .addClass('has-error');
        }
        if (json['amount'] && !$("#amount").parent().parent().hasClass("has-error")) {
          $('#amount').after('<div class="text-danger">' + json['amount'] + '</div>').parent().parent()
              .addClass('has-error');
        }
      }
      },
...
function syncOfflineOrders(i) {
  setTimeout(function () {
    $.ajax({
      ...
      type: 'post',
      data: {
        cart: pos_orders[i]['cart'],
        payment_method: pos_orders[i]['payment'],
        shipping_method: pos_orders[i]['shipping_method'],
        shipping_charge: pos_orders[i]['shipping_charge'],
        customer_id: pos_orders[i]['customer'],
        offline: 1,
        user_id: pos_orders[i]['user_login'],
        order_note: pos_orders[i]['order_note'],
        txn_id: pos_orders[i]['txn_id'],
        discount: pos_orders[i]['discount'],
        currency: pos_orders[i]['currency']
      },
      beforeSend: function () {},
      success: function (json) {
        pwidth += psegments;
...
function logout() {
  if (offline) {
    ...
    location = 'wkpos/index.php?route=wkpos/wkpos/logout';
  };
}
...
$(document).on('change', 'select[name=\'country_id\']', function () {
  ...
  // set telphone code
  if (this.options[this.selectedIndex] && this.options[this.selectedIndex].dataset.code) {
    var selectedCode = this.options[this.selectedIndex].dataset.code;
...
function registerCustomer(thisthis, select) {
  ...
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/customer/addCustomer',
    data: customer_data,
    dataType: 'json',
    type: 'post',
    beforeSend: function () {
    ...
    success: function (json) {
      if (json['success']) {
        ...
        var addCustomerForm = document.querySelector('form.addCustomer');
      ...
      if (json['error']) {
        for (field in json) if (field != 'error') {
          $('[name="' + field + '"]').after('<div class="text-danger">' + json[field] + '</div>').parent().parent().addClass('has-error');
        }
...
$(document).on('click', '#addCouponbtn', function () {
  ...
  if (offline) {
    ...
    $.ajax({
      url: 'wkpos/index.php?route=wkpos/order/applyCoupon',
      dataType: 'json',
      type: 'post',
      data: {
        coupon: coupon_code,
        customer: customer_id,
        subtotal: uf_sub_total,
        cart: pos_cart[current_cart]
      },
      beforeSend: function () {
        $('#loader').css('display', 'block');
      ...
      success: function (json) {
        if (json['success']) {
          ...
          if (json['coupon']['type']) {
            ...
            if (coupon_disc > (total - uf_total_discount + cart_tax + delivery_charge)) {
              coupon_disc = (total - uf_total_discount + cart_tax + delivery_charge);
            }
...
function makeRequest() {
  ...
  if (error || !check) {
    ...
    $.ajax({
      url: 'wkpos/index.php?route=wkpos/supplier/addRequest',
      dataType: 'json',
      type: 'post',
      data: {
        request_data: request_data,
        comment: $('#input-extrainfo').val()
      },
      beforeSend: function () {
        $('#loader').css('display', 'block');
      },
...
function getAllSuppliers() {
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/supplier',
    dataType: 'json',
    type: 'get',
    success: function (json) {
...
function getRequestHistory() {
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/supplier/getRequestHistory',
    dataType: 'json',
    type: 'get',
    success: function (json) {
...
function quantityUpdate(i) {
  var current_quantity = pos_cart[current_cart][i]['quantity'];
  var product_name = pos_cart[current_cart][i]['name'];
  $('#update-label').text(product_name);
  $('#update-quantity').prop('value', current_quantity);
  $('#cart-index').val(i);
}
...
$(document).on('click', '#updateQuantitybtn', function () {
  ...
  if (isNaN(quantity)) {
    $.toaster({
      priority: 'danger',
      message: error_quantity,
      timeout: 3000
    });
    return false;
  }

  cart_product.update(quantity, ' + pos_cart[current_cart][i]["remove"] + ', ' + pos_cart[current_cart][i]["product_index"] + ');
...
$(document).on('click', '#button-return', function () {
  ...
  for (var i = 0; i < Object.keys(pos_returns).length; i++) {
    ...
    return_html += '  <div class="table-responsive table-order order-total-table">';
...
$(document).on('change', '.return-checkbox', function () {
  ...
  return_order_product_ids.sort(function (a, b) {
    return a - b
  });
});
    ...
    if (screen.width > 767) {
      ...
      $.each(orders, function (index, order) {
        ...
        if (order['order_id'] == return_order_id) {
          ...
          return_form += '   <td><input type="text" class="form-control" id="cfirstname" value="' + customername[0] + '"/></td>';
          return_form += '   <td><input type="text" class="form-control" id="clastname" value="' + customername[1] + '"/></td>';
          return_form += '   <td><input type="text" class="form-control" id="cemail" value="' + order['email'] + '"/></td>';
          return_form += '   <td><input type="text" class="form-control" id="ctel" value="' + order['telephone'] + '"/></td>';
...
function returnProducts() {
  ...
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/return/addReturn',
    type: 'post',
    dataType: 'json',
...
function getAllExpenses(){
  $.ajax({
    url: 'wkpos/index.php?route=wkpos/expense' ,
    dataType: 'json',
    type: 'post',
    data: {
    },
    beforeSend: function () {
      $('#expenses').hide();
      $('.expenses-table').show();
      $('.expenses-table').html(`<div id="loading-text">${text_loading_expenses}</div>`);
    },
    success: function (json) {
      showExpenses(json,currency_code);
    },
    error: function () {
      $('.progress-bar').addClass('progress-bar-danger').css('width', '40%');
      $('#error-text').append('<br>' + error_load_products + '<br>');
...
$(document).on('click', '#button-credit', function () {
  ...
  $('.text-danger').remove();
  if (credit_amount > 0) {
    $(this).parent().parent().append('<button id="remove-credit" class="btn btn-sm btn-danger pull-right">' + button_remove_credit + '</button>');
    $.toaster({
      priority: 'success',
...
$(document).ajaxSuccess(function (event, response, setting) {
  if (setting.url == 'wkpos/index.php?route=wkpos/return/addReturn') {
    getAllReturns();
  }
});
$(document).ready(function () {
  $('.ar-date').datetimepicker({
    pickTime: false,
    format:'YYYY-MM-DD',
