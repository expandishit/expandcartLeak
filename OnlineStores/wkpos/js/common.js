...
var cart_product = {
  'add': function (product_index, quantity, option, thisthis, by_barcode) {
    ...
    for (var i = 0; i < Object.keys(pos_cart[current_cart]).length; i++) {
      ...
      if ((pos_cart[current_cart][i]['product_id'] == product_id) && (cart_options_string == cart_options_string1)) {
        ...
        pos_cart[current_cart][i]['uf_total'] = parseFloat(pos_cart[current_cart][i]['quantity'] * parseFloat(pos_cart[current_cart][i]['uf']).toFixed(2)).toFixed(2);
    ...
    if (skip == false) {
      ...
      uf_price = parseFloat(pos_products[product_index]['price_uf']) + parseFloat(option_price);
      product_quantity = 1;
      ...
      if(option_price != 0){
        product_price =uf_price + currency_code;
      }
      var new_product = {
        name: product_name,
        price: product_price,
  ...
  'update': function (type, key, product_index, quantity = 0) {
    var current_cart_length = Object.keys(pos_cart[current_cart]).length;
    if (current_cart_length) {
      for (var l = 0; l < current_cart_length; l++) {
        if (key == pos_cart[current_cart][l]['remove']) {
          if (type) {
            if (typeof (pos_products[product_index]) != 'undefined' && (pos_products[product_index]['quantity'] < (pos_cart[current_cart][l]['quantity'] + 1) || (pos_products[product_index]['quantity'] < quantity))) {
              $.toaster({
                priority: 'danger',
  ...
  delete: function (toastIt, cart_id) {
    ...
    if (confirm_delete) {
      ...
      if (total_cart_count <= 1) {
        ...
        $('#delivery-charge').val('');
        delivery_charge = 0;
        localforage.removeItem('delivery_charge');
        cartlocalStorage();
        showAllCarts();
        printCart();
        return;
...
function printCart(noupdate) {
  ...
  for (var i = 0; i < current_cart_length; i++) {
    ...
    if ((tax_status != 0) && (pos_cart[current_cart][i]['product_index'] != 0)) {
      var product_tax = calculateTax(pos_cart[current_cart][i]['product_id'], parseFloat(pos_cart[current_cart][i]['uf']).toFixed(2));
      cart_tax += parseFloat(product_tax * pos_cart[current_cart][i]['quantity']);
    }
    
    uf_sub_total += parseFloat(pos_cart[current_cart][i]['uf']).toFixed(2) * pos_cart[current_cart][i]['quantity'];
  
    cart_html += '<tr>';
    if (pos_cart[current_cart][i]['product_index'] == 0 || !pos_products[pos_cart[current_cart][i]["product_index"]]) {
    ...
    cart_html += '  </td>';
    cart_html += '  <td class="text-center" title="' + heading_price_update + '" data-toggle="modal" data-target="#updatePrice" onclick="priceUpdate(' + i + ')" style="cursor: pointer;">';
    if (pos_cart[current_cart][i]['special']) {
      if (typeof (pos_products[pos_cart[current_cart][i]['product_index']]) != 'undefined' && (parseFloat(pos_cart[current_cart][i]['uf']).toFixed(2)) < pos_products[pos_cart[current_cart][i]['product_index']]['price_uf']) {
  ...
  var uf_sub_total_after_discount = uf_sub_total - uf_total_discount
  // calculate tax if discount is avaiable or not

  var final_cart_tax = cart_tax;//(uf_sub_total_after_discount * cart_tax ) / uf_sub_total

  if(Number.isNaN(final_cart_tax)){
    final_cart_tax = 0;
  }

  if(uf_sub_total <= 0)
  {
    taxes_after_discount = 0
  }
  ...
  uf_cart_total = parseFloat(final_cart_tax) + parseFloat(uf_sub_total);
  ...
  fcart_tax = formatPrice(final_cart_tax);
...
function calculateTax(product_id, price) {
    var tax = 0;
    var tax_class_id = 0;
    if(pos_products[product_id]){
      tax_class_id = pos_products[product_id]['tax_class_id'];
    }
