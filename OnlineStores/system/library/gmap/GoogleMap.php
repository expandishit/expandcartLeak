...
class GoogleMapAPI {
    ...
    function getHeaderJS() { 
        ...
        $scriptUrl = "https://maps.google.com/maps/api/js?key=" . $this->api_key . "&sensor=".(($this->mobile==true)?"true":"false");
	...
	*/
    ...
    function getOnLoad() {
		...
		/*	Bug:- was not working on IE-8 
			Reported & Fix By:  Filipe Magalhaes <filipe.magellan@gmail.com>
		*/
		return '<script type="text/javascript">
					if (!window.addEventListener) {
					window.attachEvent("onload", onLoad'.$this->map_id.');
					}
					else {
					window.addEventListener("load",onLoad'.$this->map_id.', false);
					}
				</script>';               
    }
     ...
     */
    function getGeocode($address, $key = null) {
        ...
        if(($_geocode = $this->getCache($address)) === false) {
            ...
            if(($_geocode = $this->geoGetCoords($address, 0, $key)) !== false) {
    ...
    // ATH was here at 4:50 pm 8-5-2019
    function googleUrl($address, $key){

        if(!$key){
            
           return sprintf('https://%s/maps/api/geocode/json?sensor=%s&address=%s',$this->lookup_server['GOOGLE'], $this->mobile==true?"true":"false", rawurlencode($address));
        }
        
        return  sprintf('https://%s/maps/api/geocode/json?sensor=%s&address=%s&key=%s',$this->lookup_server['GOOGLE'], $this->mobile==true?"true":"false", rawurlencode($address), rawurlencode($key) );
     ...
     */
    function geoGetCoords($address,$depth=0, $key = null ) {

        switch($this->lookup_service) {
            
            case 'GOOGLE':
               
                $_url = $this->googleUrl($address, $key);
