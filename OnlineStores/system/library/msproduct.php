...
class MsProduct extends Model {
	...
	public function getCategories($parent_id = 0, $opts = []) {
		//$category_data = $this->cache->get('category.' . (int)$this->config->get('config_language_id') . '.' . (int)$parent_id);
		$category_data = FALSE;

		$restrict_categories = $this->config->get('msconf_restrict_categories');
		...
		if (!$category_data) {
			...
			if (
				isset($opts['hide_restricted']) &&
				$opts['hide_restricted'] == 1 &&
				is_array($restrict_categories) != false &&
				count($restrict_categories) > 0
			) {
				$sql .= " AND c.category_id NOT IN (" . implode(',', $restrict_categories) . ")";
			}
	...
	public function getSellerName($seller_id) {
		$sql = "SELECT nickname FROM " . DB_PREFIX . "ms_seller
				WHERE seller_id = " . (int)$seller_id;
				
		$res = $this->db->query($sql);
		
		if (isset($res->row['nickname']))
			return $res->row['nickname'];
		else
			return 0;
	}
	...
	public function saveProduct($data) {
		...
		$weight_class_id = isset($data['product_weight_class_id']) ? $data['product_weight_class_id'] : 0;
		$weight = (isset($data['product_weight']) && $data['product_weight']!="") ? $data['product_weight']:0;
		$length_class_id = isset($data['length_class_id']) ? $data['length_class_id'] : 0;
		$length = (isset($data['product_length']) && $data['product_length']!="") ? $data['product_length']:0;
		$width = (isset($data['product_width']) && $data['product_width']!="") ? $data['product_width']:0;
		$height = (isset($data['product_height']) && $data['product_height']!="") ? $data['product_height']:0;
        $cost = isset($data['product_cost']) ? $data['product_cost'] : '';
        // if not found quantity data set product unlimited quantity
        $quantity = isset($data['product_quantity']) ? $data['product_quantity'] : 1;
        $subtract = isset($data['product_quantity']) ? $data['product_subtract'] : 0;
        $unlimited = isset($data['product_quantity']) ? 0 : 1;
		...
		$sql = "INSERT INTO " . DB_PREFIX . "product
				SET model = '" . $this->db->escape($model) . "',
					...
					cost_price = " . (float)$this->MsLoader->MsHelper->uniformDecimalPoint($data['product_cost']) . ",
					...
					minimum = " . (int)$data['product_minimum'] . ",
		...
		foreach ($data['languages'] as $language_id => $language) {
			...
			$sql = "INSERT INTO " . DB_PREFIX . "product_description
					SET product_id = " . (int)$product_id . ",
						...
						slug = '" . $this->db->escape((new Slugify)->slug($language['product_name'])) . "',
						...
						seller_notes = '". $this->db->escape($language['seller_notes']) ."',
		...
		$sql = "INSERT INTO " . DB_PREFIX . "ms_product
				SET product_id = " . (int)$product_id . ",
					...
					. ( (isset($data['product_file']) && $data['product_file'] != NULL ) ? ", product_file = '" . $this->db->escape($data['product_file']) . "'" : "");
		...
		if (isset($data['product_attribute_data'])) {
			foreach ($data['product_attribute_data'] as $product_attribute) {
	...
	public function editProduct($data) {
		...
		isset($data['product_weight_class_id']) ? $included_field_sql .= " weight_class_id = " . $this->db->escape($data['product_weight_class_id']) . ","  : '';
        isset($data['product_weight']) ? $included_field_sql .= " weight = " . $this->db->escape($data['product_weight']) . ","  : '';  
		isset($data['product_length']) ? $included_field_sql .= " length = " . $this->db->escape($data['product_length']) . ","  : '';  
		isset($data['product_length_class_id']) ? $included_field_sql .= " length_class_id = " . $this->db->escape($data['product_length_class_id']) . ","  : '';
        isset($data['product_width']) ? $included_field_sql .= " width = " . $this->db->escape($data['product_width']) . ","  : '';  
        isset($data['product_height']) ? $included_field_sql .= " height = " . $this->db->escape($data['product_height']) . ","  : '';  
		...
		isset($data['product_cost']) ? $included_field_sql .= " cost_price = " . (float)$this->MsLoader->MsHelper->uniformDecimalPoint($data['product_cost']) . ","  : '';
		...
		if (isset($data['product_image']) && !empty($data['product_image'])) {
            ...
            $new_images = $data['product_image'];
            $new_images_links = [];
            foreach ($data['product_image'] as $key => $value) {
                $new_images_links[] = $value['image'];
            }
			
			foreach($old_images as $k => $old_image) {
				$key = array_search($old_image['image'], $new_images_links);
				if ($key !== FALSE) {
					unset($old_images[$k]);
			...
			foreach ($data['product_image'] as $key => $product_image) {
				$newImagePath = $this->MsLoader->MsFile->moveImage($product_image['image']);
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape(html_entity_decode($newImagePath, ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$product_image['sort_order'] . "'");
			}
                        ...
                        if (isset($product_option_value['images'])) {
                            foreach ($product_option_value['images'] as $image) {
                                if (is_array($image) && isset($image['image']) && $image['image']) {
                                    $product_options_images[] = $image['image'];
                                }
                            }
                        }
                    }
		...
		foreach($old_images as $old_image) {
			if ($old_image['image'] != $thumbnail && !in_array($old_image['image'], $product_options_images)) {
    ...
    {
		return $this->countProducts($seller_id, $product_id);			
    }
    
    public function countProducts($seller_id, $product_id = null)
    {
        $sql = "SELECT COUNT(*) as 'total'
				FROM `" . DB_PREFIX . "ms_product`
                WHERE seller_id = " . (int)$seller_id
                .($product_id ? (" AND product_id = " . (int)$product_id) : "");
	...
	public function getProduct($product_id) {
		$sql = "SELECT 	p.price,
						p.cost_price,p.weight,p.weight_class_id,
		...
		foreach ($descriptions->rows as $result) {
			$product_description_data[$result['language_id']] = array(
				...
				'seller_notes'     => $result['seller_notes']
			);
		}
	...
	public function getProducts($data = array(), $sort = array(), $cols = array()) {
		...
		if(isset($sort['filters'])) {
			...
			foreach($sort['filters'] as $k => $v) {
				if (!isset($cols[$k])) {
					$wFilters .= " {$k} LIKE '%" . $this->db->escape($v) . "%' OR";
				} else {
					$hFilters .= " {$k} LIKE '%" . $this->db->escape($v) . "%' OR";
				}
			}
			$wFilters = !empty($wFilters) ? ' AND (' . substr( $wFilters, 0, -2 ) . ') ' : '';
			$hFilters = !empty($hFilters) ? ' AND (' . substr( $hFilters, 0, -2 ) . ') ' : '';
		}
		...
		$sql = "SELECT
				...
				. (isset($data['product_status']) ? " AND product_status IN  (" .  $this->db->escape(implode(',', $data['product_status'])) . ")" : '')
				. (isset($data['cartProductsIDs']) ? " AND p.product_id IN (" .  $this->db->escape(implode(',', $data['cartProductsIDs'])) . ")" : '')
				. $wFilters
	...
	public function getProductFile($product_id){
		$sql = "SELECT product_file FROM ". DB_PREFIX . " ms_product WHERE product_id = '" . (int)$product_id . "'";
		$res = $this->db->query($sql);
		return $res->row['product_file'];
	}
}
?>
