<?php
class MsSms extends Model
{

    /**
     * Send seller smshare messages
     *
    ...
    private function checkIfCanSendSms($callback)
    {
        ...
        $app_status = $this->model_module_smshare->isInstalled() && (int)$this->config->get('smshare_config_notify_seller_on_status_change') === 1;
    ...
    private function mapMessages($app_status, $messages, $callback)
    {
        return array_map(function ($message) use ($app_status, $callback) {
            ...
            if ($app_status === true) {
                ...
                $message['status'] = false;
                $message['message'] = 'service not installed or smshare config for seller not enabled';
            }
            return $message;
        }, $messages);
    }
    ...
    private function sendMessage(array $message)
    {
        switch ($message['type']) {
            ...
            case MsSeller::STATUS_NOPAYMENT:
                return $this->model_module_smshare->notify_or_not_on_admin_change_seller_status($message);
        }
