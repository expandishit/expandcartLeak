...
class Payfort_Fort_Helper
{
    ...
    public function calculateSignature($arrData, $signType = 'request')
    {
        ...
        foreach ($arrData as $k => $v) {
            // $k == products case is handeled only for valu 
            if($k == "products"){
                $shaString .= "products=[{product_name=".$v[0]['product_name'].", product_price=".$v[0]['product_price'].", product_category=".$v[0]['product_category']."}]";
            }
            else{
                // this for any other payfort methods such as cc, naps or sadad
                $shaString .= "$k=$v";
            }
        }
        if ($signType == 'request') {
    ...
    public function getValuGatewayUrl()
    {
        $testMode = $this->pfConfig->isSandboxMode();
