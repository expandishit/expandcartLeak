...
try {
	...
	require_once(ONLINE_STORES_PATH.'OnlineStores/admin/controller/module/mart.php');

	$mart_controller = new ControllerModuleMart($registry);

	$operation = $_SERVER['argv'][1];
	if($operation == 'custom'){
        $page_from = $_SERVER['argv'][3];
        $page_to = $_SERVER['argv'][4];
        $mart_controller->fetchProductsFromCustomPages($page_from,$page_to);
    }else{
        $mart_controller->fetchAllProducts();
    }
