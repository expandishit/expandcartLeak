<?php
class MsWhatsappCloud extends Model
{
    ...
    {
        return $this->checkIfCanSendMessage(function ($app_status) use ($messages) {
            return $this->sendMessages($app_status, $messages);
        });
    }

    private function checkIfCanSendMessage($callback)
    {
        $this->load->model('module/whatsapp_cloud');
        $this->language->load('sale/customer');
        $app_status = $this->model_module_whatsapp_cloud->isInstalled() && (int)$this->config->get('whatsapp_cloud_config_notify_seller') === 1;
    ...
    private function sendMessage(array $message)
    {
        switch ($message['type']) {
            ...
            case MsSeller::STATUS_NOPAYMENT:
                return $this->model_module_whatsapp_cloud->sellerStatusNotification($message);
        }
