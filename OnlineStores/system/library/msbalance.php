...
class MsBalance extends Model {
	...
	public function sellerBalanceExistsForOrderProduct(int $order_id, int $seller_id, int $product_id)
	{
		$result = $this->db->query("SELECT * FROM ". DB_PREFIX . "ms_balance WHERE `seller_id` = '{$seller_id}' AND `order_id` = '{$order_id}' AND `product_id` = '{$product_id}' LIMIT 1");
