...
class Length {
	...
	public function getClassByUnit(string $unit)
	{
		foreach($this->lengths as $id => $length){
			if(strcasecmp(trim($length['unit']) , trim($unit)) == 0)
				return $length;
		}
	}

	public function getUnit($length_class_id, $langId = null) {
        if (is_null($langId)){
            if (isset($this->lengths[$length_class_id])) {
        ...
        $getLength = $this->db->query("SELECT * FROM " . DB_PREFIX . "length_class mc LEFT JOIN " . DB_PREFIX . "length_class_description mcd ON (mc.length_class_id = mcd.length_class_id) WHERE mc.length_class_id = '" . (int) $length_class_id . "' and mcd.language_id = '" . (int) $langId . "'");

        return $getLength->row['unit'] ?? '';
	}		
}
?>