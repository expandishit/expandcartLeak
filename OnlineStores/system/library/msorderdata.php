...
class MsOrderData extends Model {
	public function getOrders($data = array(), $sort = array(), $cols = array()) {
        ...
        if (count($data['order_status']) > 1){
            $order_statuses = implode(',',$data['order_status']);
        }
        else
        {
            $order_statuses = $data['order_status'][0];
        }
		$sql = "SELECT
					SQL_CALC_FOUND_ROWS
					*,"
		...
		. (count($data['order_status']) > 0 ? " AND o.order_status_id IN  (" .  $this->db->escape($order_statuses) . ")" : '')
	...
	*/
	...
	public function getTotalSales($data = array()) {
		...
		if (count($this->config->get('msconf_credit_order_statuses')) > 1){
			$msconf_credit_order_statuses = implode(',',$this->config->get('msconf_credit_order_statuses'));
		}
		else
		{
            $msconf_credit_order_statuses = $this->config->get('msconf_credit_order_statuses')[0];
		}
		...
		$sql = "SELECT SUM(quantity) as 'total' FROM (
					...
					. ($this->config->get('msconf_credit_order_statuses') ? " AND o.order_status_id IN  (" . $msconf_credit_order_statuses . ")" : '')
