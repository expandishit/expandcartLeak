<?php
//  Website: WWW.OpenCartArab.com
//  E-Mail : info@OpenCartArab.com

// Heading 
$_['heading_title']                  = 'إتمام الطلب';

// Text
$_['text_cart']                      = 'سلة الشراء';
$_['text_checkout_option']           = 'الخطوة 1: خيارات إتمام الطلب';
$_['text_checkout_account']          = 'الخطوة 2: الحساب وتفاصيل الفاتورة';
$_['text_checkout_payment_address']  = 'الخطوة 2: تفاصيل الفاتورة';
$_['text_checkout_shipping_address'] = 'الخطوة 3: تفاصيل الشحن';
$_['text_checkout_shipping_method']  = 'الخطوة 4: طريقة الشحن';
$_['text_checkout_payment_method']   = 'الخطوة 5: طريقة الدفع';
$_['text_checkout_confirm']          = 'الخطوة 6: تأكيد الطلب';
$_['text_modify']                    = 'تعديل &raquo;';
$_['text_new_customer']              = 'عميل جديد';
$_['text_returning_customer']        = 'تسجيل الدخول';
$_['text_checkout']                  = 'خيارات إتمام الطلب:';
$_['text_i_am_returning_customer']   = 'انا عميل مسجل بالموقع';
$_['text_register']                  = 'تسجيل';
$_['text_guest']                     = 'إتمام الطلب للزوار';
$_['text_register_account']          = 'لكي تقوم بإتمام الطلب قم بإنشاء حساب جديد معنا، فهو يُمكنك من الشراء بصورة أسرع و متابعة طلبيات الشراء التي تقدمت بها, و مراجعة سجل الطلبيات القديمة واسعتراض الفواتير وغير ذلك الكثير....';
$_['text_forgotten']                 = 'نسيت كلمة المرور';
$_['text_your_details']              = 'معلوماتك الشخصية';
$_['text_your_address']              = 'العناوين الخاصة بك';
$_['text_your_password']             = 'كلمة المرور الخاصة بك';
$_['text_agree']                     = 'لقد قرأت ووافقت على <a class="colorbox" href="%s" alt="%s"><b>%s</b></a>';
$_['text_address_new']               = 'أريد استخدام عنوان جديد';
$_['text_address_existing']          = 'أريد استخدام هذا العنوان';
$_['text_shipping_method']           = 'الرجاء اختيار طريقة الشحن المفضلة لهذا الطلب.';
$_['text_payment_method']            = 'الرجاء اختيار طريقة الدفع المفضلة لهذا الطلب.';
$_['text_comments']                  = 'كتابة ملاحظات مع الطلب.';

// Column
$_['column_name']                    = 'الاسم';
$_['column_model']                   = 'النوع';
$_['column_quantity']                = 'الكمية';
$_['column_price']                   = 'السعر';
$_['column_total']                   = 'الاجمالي';

// Entry
$_['entry_email_address']            = 'عنوان البريد الالكتروني :';
$_['entry_email']                    = 'البريد الالكتروني :';
$_['entry_password']                 = 'كلمة المرور :';
$_['entry_confirm']                  = 'تأكيد كلمة المرور :';
$_['entry_firstname']                = 'الاسم الأول :';
$_['entry_lastname']                 = 'اسم العائلة :';
$_['entry_telephone']                = 'رقم الهاتف أو الجوال :';
$_['entry_fax']                      = 'الفاكس :';
$_['entry_company']                  = 'الشركة :';
$_['entry_customer_group']           = 'نوع الشركة :';
$_['entry_company_id']               = 'رقم معرف الشركة :';
$_['entry_tax_id']                   = 'رقم معرف الضريبة :';
$_['entry_address_1']                = 'العنوان:';
$_['entry_address_2']                = 'العنوان الثاني :';
$_['entry_postcode']                 = 'الرمز البريدي :';
$_['entry_city']                     = 'المدينة :';
$_['entry_country']                  = 'البلد :';
$_['entry_zone']                     = 'المنطقة / المحافظة :';
$_['entry_location']                 = 'موقع الشحن :';
$_['entry_newsletter']               = 'أريد الاشتراك في  %s القائمة البريدية.';
$_['entry_shipping'] 	             = 'عنوان الشحن وعنوان الدفع الخاص بي متطابق.';
$_['send_as_gift']                   = 'إرسال كهدية.';

// Error
$_['error_warning']                  = 'توجد مشكلة أثناء تنفيذ الطلب، اذا تكررت معك نفس المشكاة الرجاء اختيار طريقة دفع مختلفة أو الاتصال بنا <a href="%s"> بالنقر هنا </a>.';
$_['error_login']                    = 'تحذير : لا يوجد تطابق مع البريد الإلكتروني و / أو كلمة المرور.';
$_['error_login_username']           = 'تحذير : لا يوجد تطابق مع اسم المستخدم و / أو كلمة المرور.';
$_['error_approved']                 = 'تحذير : يجب الموافقة على حسابك من الادارة قبل تسجيل الدخول.';
$_['error_exists']                   = 'تحذير : البريد الالكتروني مسجل مسبقا!';
$_['error_firstname']                = 'الاسم الأول يجب أن يكون أكثر من 1 وأقل من 32 حرفا وأن يحتوي على أحرف فقط!';
$_['error_lastname']                 = 'اسم العائلة يجب أن يكون أكثر من 1 وأقل من 32 حرفا!';
$_['error_email']                    = 'البريد الإلكتروني غير صحيح الرجاء إعادة كتابته من جديد!';
$_['error_telephone']                = 'رقم الهاتف يجب أن يكون أكثر من 3 وأقل من 32 رقم!';
$_['error_password']                 = 'كلمة المرور يجب أن تكون أكثر من 3 وأقل من 20 حرفا!';
$_['error_confirm']                  = 'لم تتطابق كلمة المرور الرجاء إعادة كتبته مرة أخرى!';
$_['error_company_id']               = 'رقم معرف الشركة مطلوب !';
$_['error_tax_id']                   = 'رقم معرف الضريبة مطلوب !';
$_['error_vat']                      = 'رقم معرف الضريبة غير صحيح !';
$_['error_address_1']                = 'العنوان يجب أن يكون أكثر من 1 وأقل من 128 حرفا!';
$_['error_city']                     = 'اسم المدينة يجب أن يكون أكثر من 3 وأقل من 128 حرفا';
$_['error_postcode']                 = 'الرمز البريدي يجب أن يكون أكثر من 2 وأقل من 10 رمزاً !';
$_['error_country']                  = 'الرجاء اختيار البلد!';
$_['error_zone']                     = 'الرجاء اختيار المنطقة / المحافظة!';
$_['error_agree']                    = 'تحذير : يجب أن توافق على %s!';
$_['error_address']                  = 'تحذير : يجب اختيار عنوان!';
$_['error_shipping']                 = 'تحذير: طريقة الشحن مطلوبة!';
$_['error_no_shipping']              = 'تحذير: خدمات الشحن لدينا لا تغطي المنطقة المختارة. الرجاء <a href="%s">الاتصال بنا</a> لمساعدتك!';
$_['error_payment']                  = 'تحذير: طريقة الدفع مطلوبة!';
$_['error_no_payment']               = 'تحذير: لا يوجد طرق دفع متاحة لإجمالي الطلب الحالي أو أن طرق الدفع المتاحة لا تغطي المنطقة المختارة. الرجاء <a href="%s">الاتصال بنا</a> لمساعدتك!';
$_['error_send_as_gift']             = 'تحذير : يجب أن توافق على %s!';
?>
