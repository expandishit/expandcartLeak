<div class="box_rewardpoints">
    ...
    <div style="float:left;min-width: 99%;padding: 10px 0;">
        ...
        <div style="margin: 40px 10px 19px 12px" class="box_check_use_maxium">
            <input type="checkbox" class="use_maxium" <?php echo (isset($this->session->data['points_to_checkout']) && $this->session->data['points_to_checkout'] == $max_redeem_point ? 'checked="checked"' : '')?> value="<?php echo $max_redeem_point?>"/>
            <?php echo sprintf( $language->get('entry_maximum_redeem'), number_format($max_redeem_point), $config->get('text_points_'.$language->get('code')) )?></span>
        </div>
        <?php } else { ?>
