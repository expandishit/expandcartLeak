...
$helper->addScript( 'catalog/view/javascript/nabar.js' );
...
if( file_exists(DIR_TEMPLATE.$themeName.'/stylesheet/customize/'.STORECODE.'/clientcustomstyle.css') ) {
$helper->addCss( 'catalog/view/theme/'.$themeName.'/stylesheet/customize/'.STORECODE.'/clientcustomstyle.css?'.microtime()  );
}
...
<head>
    ...
    <style>
        body {
            background: url("image/<?php echo STORECODE . '/' . $themeConfig['bg_image'];?>") < ? php echo $ themeConfig [ 'bg_repeat' ];
            ? > < ? php echo $ themeConfig [ 'bg_position' ];
            ? > !important;
        }</style>
    <?php } ?>
    <?php
    ...
    $css[]=
    trim($themeConfig['body_selector'.$i])." {font-family:".str_replace("'",'"',htmlspecialchars_decode(trim($themeConfig['normal_fonts'.$i])))."}
    \r\n" ;
    }
    }
    echo implode( "\r\n",$link );
    ...
    <script type="text/javascript"><!--
        $(document).ready(function () {
            <
            ? php echo
            html_entity_decode(trim($themeConfig['custom_javascript']));
            ?
            >
        });
        //--></script>
...
<body id="offcanvas-container"
      class="offcanvas-container layout-<?php echo $layoutMode; ?> fs<?php echo $themeConfig['fontsize'];?> <?php echo $helper->getPageClass();?> <?php echo $helper->getParam('body_pattern','');?>">
                ...
                <div class="row">
                    <div class="col-lg-8 col-md-6 col-sm-5">
                        <ul class="links hidden-sm hidden-xs  pull-left">
                            <li><a href="<?php echo $wishlist; ?>" id="wishlist-total"><span
                                            class="icon-heart"></span><?php echo $text_wishlist; ?></a></li>
                            <li><a href="<?php echo $account; ?>"><span
                                            class="icon-user"></span><?php echo $text_account; ?></a></li>
                            <li><a href="<?php echo $shopping_cart; ?>"><span
                                            class="icon-shopping-cart"></span><?php echo $text_shopping_cart; ?></a>
                            </li>
                            <li><a class="last" href="<?php echo $checkout; ?>"><span
                                            class="icon-file"></span><?php echo $text_checkout; ?></a></li>
                        </ul>
                        <div class="login pull-right hidden-sm hidden-xs hidden-md">
                            <?php if (!$logged) { ?>
...
<section id="pav-slideshow" class="pav-slideshow hidden-xs hidden-sm">
    <div class="container">
        ...
        <div class="row"><?php } ?>
            <div class="<?php echo $class[$j];?> <?php echo " col-sm-
            ".$ospans[$j]?>"><?php echo $module; ?></div>
        <?php if( $i%$cols == 0 || $i==count($modules) ){ ?></div>
    <?php } ?>
    <?php  $j++;  } ?>
    </div>
</section>
...
/**
 * Promotion modules
 * $ospans allow overrides width of columns base on thiers indexs. format array( column-index=>span number ), example
array( 1=> 3 )[value from 1->12]
*/
        ...
        <div class="row visible-xs visible-sm">
            <div class="container">
                <div class="offcanvas-sidebars-buttons">
                    <button type="button" data-for="column-left" class="pull-left btn btn-danger"><i
                                class="glyphicon glyphicon-indent-left"></i> <?php echo $this->
                        language->get('text_sidebar_left'); ?>
                    </button>

                    <button type="button" data-for="column-right" class="pull-right btn btn-danger"><?php echo $this->
                        language->get('text_sidebar_right'); ?> <i class="glyphicon glyphicon-indent-right"></i>
                    </button>
                </div>
            </div>
        </div>
