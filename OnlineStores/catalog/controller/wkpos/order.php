...
class ControllerWkposOrder extends Controller {
	public function index() {
        ...
        $language_info = $this->model_localisation_language->getLanguage($this->config->get('config_admin_language'));
        $language_directory = $language_info['directory'];

        $show_qr = false;
        if ($this->config->get('config_show_qr')  &&  $this->config->get('config_qr_compatible_with') == 'ksa'){
            $show_qr = true;
        }
		...
		foreach ($orders as $order) {
			...
			$datetime = explode(' ', ($this->getDateByCurrentTimeZone($order['date_added'])));
            ...
            $json['order_products'][$order['order_id']]['qr_code'] = $show_qr ?
                HTTP_SERVER . '/admin/setting/invoice/generateZatcaQR?'.
                'language_id'.$this->config->get('config_language_id').
                '&order_id='.$order['order_id'].
                '&language_code='.$this->config->get('config_admin_language').
                '&language_directory='.$language_directory :null;
        }
	...
	public function addOrder() {
		...
		if ($this->request->post['payment_method'] == 'cash') {
            ...
            $array = $this->config->get('wkpos_custom_title' . $this->config->get('config_language_id') );
            foreach($array as $key => $value){
                if ($order_data['payment_code'] == $value){
                    $order_status_id = $this->config->get('wkpos_custom_order_status_id')[$key];
                    $order_data['payment_method'] =  $value;
                }
            }
        }

		if (isset($this->request->post['partial_credit_payment']) && $this->request->post['only_credit_payment'] != 'false') {
			...
			$order_data['payment_method'] = $this->config->get('wkpos_credit_title' . $this->config->get('config_language_id'));
			$order_data['payment_code'] = 'customer_credit';
			if (!$order_data['payment_method'])
		...
		$order_data['comment'] = $this->request->post['order_note'] ?? '';
		...
		$this->model_checkout_order->confirm($order_id, $order_status_id, $this->request->post['order_note']);
