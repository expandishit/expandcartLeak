...
class ControllerAffiliateHistory extends Controller {
	public function index() {
		...
		$this->language->load('affiliate/history');
        ...
        $this->data['balanceIncPending'] = $this->currency->format($this->model_affiliate_transaction->getBalanceIncPending());
        $this->data['earning'] = $this->currency->format($this->model_affiliate_transaction->getTotalEarning());
		
		$this->data['continue'] = $this->url->link('affiliate/account', '', 'SSL');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/affiliate/history.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/affiliate/history.tpl';
		} else {
			$this->template = 'default/template/affiliate/history.tpl';
		}
