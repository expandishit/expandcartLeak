	...
	public function index(){
		...
		if($this->validate()) {	
			...
			if($this->settings['general']['enable']){
                ...
                $this->data['sms_conf_msg'] = $this->language->get('sms_conf_msg');
                $this->data['sms_conf_resend'] = $this->language->get('sms_conf_resend');
                $this->data['sms_conf_verify'] = $this->language->get('sms_conf_verify');
                $this->data['sms_conf_cancel'] = $this->language->get('sms_conf_cancel');
                $this->data['sms_conf_error'] = $this->language->get('sms_conf_error');
				
				$this->data['whats_conf_msg'] 	 = $this->language->get('whats_conf_msg');
                $this->data['whats_conf_resend'] = $this->language->get('whats_conf_resend');
                $this->data['whats_conf_verify'] = $this->language->get('whats_conf_verify');
                $this->data['whats_conf_cancel'] = $this->language->get('whats_conf_cancel');
                $this->data['whats_conf_error']  = $this->language->get('whats_conf_error');
 ...
 */
	private function get_shipping_address_view(){
		...
		$this->session->data['guest']['shipping'] = $this->session->data['shipping_address'];
                  ///  echo $data['display'].' - '. ($this->session->data['payment_address']['shipping'] == 0);
 ...
 */
	private function create_order(){
		$this->get_total_data($total_data, $total, $taxes);
			...
			if (isset($this->request->cookie['tracking']) && $this->affiliateAppIsActive()) {
 ...
 */
	public function confirm_order(){
		...
		$data = array();
            ...
            // $data['payment_address_location'] = $payment_address['location'];
...
/*				$shipping_address =  $this->model_account_address->getAddress($this->session->data['shipping_address']['address_id']);
                ...
                $data['shipping_address_location'] = $shipping_address['location'];
	...
	*/
	...
	private function get_payment_methods($payment_address){
		...
		foreach ($method_data as $key => $value) {
			$sort_order[$key] = $value['sort_order'] ? $value['sort_order'] : 99999;
		}
	...
	public function send_conf_code_sms() {
		...
		if (\Extension::isInstalled('whatsapp')) {
			...
			if($order['phoneverified'] == '0' && $order['whatsveriftrials'] > 0) {
				...
				$this->model_module_whatsapp->send_conf_sms_to_customer_on_new_order($data);
