	...
	public function jxSubmitProduct() {
		...
		foreach ($data['languages'] as $language_id => $language) {
			...
			if ($i >= 0) {
				...
				} else if (mb_strlen($language['product_name']) < 1/*4*/ || mb_strlen($language['product_name']) > 50 ) {
					$json['errors']['product_name_' . $language_id] = sprintf($this->language->get('ms_error_product_name_length'), 1/*4*/, 50);
				}
				...
				/*if (empty($language['product_description'])) {
					...
					$json['errors']['product_description_' . $language_id] = sprintf($this->language->get('ms_error_product_description_length'), 0/*25*/, 4000);
				}
			} /*else {
