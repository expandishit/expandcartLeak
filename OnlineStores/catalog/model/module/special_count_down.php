<?php
class ModelModuleSpecialCountDown extends Model {
	...
	$query = $this->db->query("SELECT MAX(price_bid) as max_price_bid FROM " . DB_PREFIX . "bids_history WHERE product_id=".(int)$product_id." LIMIT 1");		if (isset($query->row['max_price_bid'])){		$max_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "bids_history WHERE price_bid=".$query->row['max_price_bid']."");		}				if (isset($max_query->row['price_bid']) && isset($max_query->row['customer_id'])) {			return array($max_query->row['customer_id'],$max_query->row['price_bid'],$max_query->row['firstname'],$max_query->row['date_added']);				}else{			return 0;				}							
	...
	public function getProductSpecialssuh($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
		...
		$sql = "SELECT ps.* FROM " . DB_PREFIX . "product_bid ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) 
		WHERE p.status = '1' AND ps.auction_status = '1' GROUP BY ps.product_bid_id";
	...
	public function getTotalProductSpecials() {
		...
		public function getSpecialsEndDate($product_id) {	
		$query = $this->db->query("SELECT date_end FROM " . DB_PREFIX . "product_special 
