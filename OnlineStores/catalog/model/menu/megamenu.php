...
class ModelMenuMegamenu extends Model {		
	...
	private $colname = CURRENT_TEMPLATE == 'pav_bestbuy' ? 'span' : 'col-sm-';
    private $rowname = CURRENT_TEMPLATE == 'pav_bestbuy' ? 'row-fluid' : 'row';
		...
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				...
				if( $this->hasChild($menu['megamenu_id']) || $menu['type_submenu'] == 'html'){
					...
					if( $menu['image']){ $output .= '<span class="menu-icon" style="background:url(\''.HTTP_IMAGE.$menu['image'].'\') no-repeat;">';	}
					if($menu['show_title']) {
	...
	public function genMegaMenuByConfig( $parentId, $level,$menu  ){
		...
		foreach( $menu['megaconfig']->rows  as $row ){
			...
			$output .= '<div class="' . $this->rowname . '">';
				foreach( $row->cols as $col ){
					$colclass = isset($col->colclass)?$col->colclass:'';
					 $output .= '<div class="mega-col '.$this->colname.$col->colwidth.' '.$colclass.'" '.$this->getColumnDataConfig( $col ).'> <div class="mega-col-inner">';
		...
		if( $parent['type_submenu'] == 'html' ){
			...
			if( $parent['colums'] > 1  ){
				...
				if( !empty($parent['megaconfig']->rows) ) {
						...
						foreach( $rows as $rowcols ){
							...
							foreach( $rowcols as $key => $col ) {
								...
								if( isset($col->type) && $col->type == 'menu' && isset($cols[$key]) ){
									$scol = '<div class="mega-col '.$this->colname.$col->colwidth.'" data-type="menu" '.$this->getColumnDataConfig( $col ).'><div class="mega-col-inner">';
									$scol .= '<ul>';
									...
									$scol = '<div class="mega-col '.$this->colname.$col->colwidth.'"  '.$this->getColumnDataConfig( $col ).'><div class="mega-col-inner">';
					...
					$output = '<div class="'.$class.' mega-cols cols'.$parent['colums'].'" '.$attrw.' ><div class="dropdown-menu-inner"><div class="' . $this->rowname . '">';
					$cols   = array_chunk( $data, ceil(count($data)/$parent['colums'])  );
			...
			///	echo '<pre>' .print_r( $parent, 1 );
				if( !empty($parent['megaconfig']->rows) ) {
						...
						foreach( $rows as $rowcols ){
							...
							foreach( $rowcols as $col ) {
								...
								if( isset($col->type) && $col->type == 'menu' ){
									...
									$scol = '<div class="mega-col '.$this->colname.$colwidth.'" data-type="menu" '.$this->getColumnDataConfig( $col ).'><div class="mega-col-inner">';
									$scol .= '<ul>';
									foreach( $data as $menu ){
										$scol .= $this->renderMenuContent( $menu , $level+1 );
									}	
					...
					$output = '<div class="'.$class.' level'.$level.'" '.$attrw.' ><div class="dropdown-menu-inner">';
					$row = '<div class="' . $this->rowname . '"><div class="' . $this->colname . '12 mega-col" data-colwidth="12" data-type="menu" ><div class="mega-col-inner"><ul>';
					foreach( $data as $menu ){
	...
	public function getColWidth( $menu, $cols ){
		...
		for( $i=1; $i<= $cols; $i++ ){
			if( array_key_exists($i,$output) ){
				$spans[$i] = $this->colname.$output[$i];
			}else{		
				if( (12-$tmp)%($cols-count($output)) == 0 ){
					$spans[$i] = $this->colname.((12-$tmp)/($cols-count($output)));
				}else {
					if( $t == 0 ) {
						$spans[$i] = $this->colname.( ((11-$tmp)/($cols-count($output))) + 1 ) ;
					}else {
						$spans[$i] = $this->colname.( ((11-$tmp)/($cols-count($output))) + 0 ) ;
					}
					$t++;
				}					
			}
