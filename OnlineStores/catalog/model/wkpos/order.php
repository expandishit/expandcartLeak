 ...
 */
  ...
  */
  public function addUserOrders($pos_data) {
    $user = $this->db->query("SELECT CONCAT(firstname, ' ', lastname) as name FROM " . DB_PREFIX . POS_USERS_TABLE . " WHERE user_id = '" . (int)$pos_data['user_id'] . "'")->row;
    $description = 'Order ID #'.$pos_data['order_id'];
    $this->db->query("INSERT INTO " . DB_PREFIX . "wkpos_user_orders SET order_id = '" . (int)$pos_data['order_id'] . "', user_id = '" . (int)$pos_data['user_id'] . "', user_name = '" . $this->db->escape($user['name']) . "', order_note = '" . $this->db->escape($pos_data['note']) . "', txn_id = '" . $pos_data['txn_id'] . "'");
    if (isset($pos_data['credit']) && $pos_data['credit'] > 0) {
      $this->db->query("INSERT INTO " . DB_PREFIX . "wkpos_credit SET amount=" . -(float)$pos_data['credit'] . ", customer_id=".(int)$pos_data['customer_id'] .", description = '" . $this->db->escape($description) . "', date_added = NOW()");
    }
    if (isset($pos_data['add_credit']) && $pos_data['add_credit'] > 0) {
      $this->db->query("INSERT INTO " . DB_PREFIX . "wkpos_credit SET amount=" . (float)$pos_data['add_credit'] . ", customer_id=".(int)$pos_data['customer_id'] .", description = '" . $this->db->escape($description) . "', date_added = NOW()");
    }
  }
}
