...
class ModelWkposUser extends Model {
  ...
  */
  public function getUser($user_id, $user_only = '') {
    $user_query = $this->db->query("SELECT * FROM " . DB_PREFIX . POS_USERS_TABLE . " WHERE user_id = '" . $user_id . "'");
  ...
  */
  public function getUsersByEmail($email) {
    $sql = "SELECT count(*) as total FROM " . DB_PREFIX . POS_USERS_TABLE . " WHERE email = '" . $email . "' AND user_id != '" . $this->session->data['user_login_id'] . "'";
  ...
  */
  public function checkPreviousPwd($pwd) {
    $user_query = $this->db->query("SELECT * FROM " . DB_PREFIX . POS_USERS_TABLE . " WHERE (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($pwd) . "'))))) OR password = '" . $this->db->escape(md5($pwd)) . "') AND status = '1' AND user_id = '" . $this->session->data['user_login_id'] . "'");
  ...
  */
  public function updateProfile($data) {
    $this->db->query("UPDATE " . DB_PREFIX . POS_USERS_TABLE . " SET firstname = '" . $data['firstname'] . "', lastname = '" . $data['lastname'] . "', email = '" . $data['account_email'] . "', username = '" . $data['username'] . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['account_npwd'])))) . "' WHERE user_id = '" . $this->session->data['user_login_id'] . "'");
  }
  /**
   ...
   */
	public function checkUserLogin() {
		if (isset($this->session->data['user_login_id']) && $this->session->data['user_login_id']) {
			$check_user = $this->db->query("SELECT * FROM " . DB_PREFIX . POS_USERS_TABLE . " WHERE user_id = '" . $this->session->data['user_login_id'] . "'")->row;
			return $check_user;
		} else {
			return false;
		}
