...
class ModelWkposReturn extends Model {
	public function addReturn($data) {
		$check = $this->db->query("SELECT * FROM `" . DB_PREFIX . "return` where `order_id` = " . (int)$data['order_id'] . " and `product_id` = ".(int)$data['product_id'].";");
		if($check->num_rows == 0 ){
			...
			$orderProduct = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_product` WHERE `order_id` = ". (int)$data['order_id'] ." AND `order_product_id` = " . (int)$data['product_id'] . ";");    
			
			//Update the total order
			$this->db->query("UPDATE `".DB_PREFIX."order` SET `total` = `total` - " . $orderProduct->row['price'] . " where `order_id` = ".$data['order_id'].";");
		
			//update order product with the new quantity or delete it when it will become zero
			if( ($orderProduct->row['quantity'] - $data['quantity']) == 0){
				$this->db->query("DELETE from `" . DB_PREFIX . "order_product` WHERE `order_id` = ". $data['order_id'] ." AND `product_id` = " . $data['product_id'] . ";");
			}else{
				$this->db->query("UPDATE `".DB_PREFIX."order_product` SET `quantity` = `quantity` - ".$data['quantity']." WHERE `order_id` = ". $data['order_id'] ." AND `product_id` = " . $data['product_id'] . ";");
			}
