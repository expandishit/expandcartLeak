  ...
  <div class="box">
    ...
    <?php if($productsoptions_sku_status) { ?>
        <div id="tab-variation-sku">
            <?php if(isset($option_combinations) && !empty($option_combinations)) {
                ?>
            <table class="table table-hover dataTable no-footer">
                <thead><tr>
                    <?php $count=count($option_combinations[0]);
                                        $i=0;
                                        echo "<td>#</td>";
                    foreach($option_combinations as $key=>$option_combination) {
                    usort($option_combination, 'sortByOrder');
                    if($i==$count) {
                    break;
                    }
                    echo "<td class='left'>".$option_combination[$i]['name']."</td>";
                    $i++;
                    } ?>
                    <td>SKU</td></tr>
                </thead>
                <tbody>
                <input type="hidden" name='num_options' value="<?php echo $count;?>">
                <?php $j=1;

                foreach ($option_combinations as $keys=>$option_combination) { $option_value_ids='';?>
                <tr>
                    <td class='left'><?php echo $j;?></td>
                    <?php usort($option_combination, 'sortByOrder');
                    foreach($option_combination as $key=>$values) {?>
                    <td class='left'>
                    <?php
                    echo $option_combination[$key]['value'];
                    $option_value_ids .=$option_combination[$key]['option_value_id'].',';

                    ?>
                    <input type='hidden' name='option_variation_value[<?php echo $keys;?>][]'
                    value='<?php echo $option_combination[$key]['option_value_id']; ?>'>
                    </td>
                    <?php }?>
                    <td class='left'>
                        <input type='text' value='<?php if(isset($product_variation_skus[substr($option_value_ids, 0 , -1)])){echo $product_variation_skus[substr($option_value_ids, 0 , -1)];}?>' name='option_variation_value[<?php echo $keys;?>][]'>
                    </td>
                </tr>
                <?php $j++;} ?>
                </tbody>
            ...
            <?php } else { ?>
            <b>There are no/sufficient options for option combinations.</b>
            <?php }?>
        </div>
