	...
	$.fn.pavoSliderEditor = function( initvar ) {
		...
		this.createList = function( JSLIST  ){
 		...
 		this.createLayer = function( type, data, slayerID ){
			var $this=this;
	 		...
	 		ilayer.append( '<span class="status">' + txt_show + '</span>' );
		...
		this.preview=function(){
				...
				var $class = $("#"+field).val();
					...
					$('#content').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><form style="display: none;" action="'+$this.adminURL+'&route=module/pavsliderlayer/preview&field='
		...
		this.createDefaultLayerData = function( layer, data ){
	 		...
	 		if( data !=null && data ) { 
		 		...
		 		if(  data['layer_type'] == 'image' ){
					...
					var src = $this.siteURL+data['layer_content'];
