...
//@ViewChild('control') control;
  ...
  set controlValue(value:any) {
    ...
    else if(this.field.Type == "product-or-category") {
      if(value == "") {
        this.field.Values.forEach((val, i) => {
        ...
        this.field.Values.forEach((val, i) => {
          this.field.Values[i].Value =  this.selectedTagType + ":" + _val;
        })
      }
    }
    else {
  ...
  ngOnInit() {
    ...
    if(this.field.Type == "product-or-category") {
      this.tagTypes = [{"Name": (this.commonService.Res$.getValue())._product, "Value": "product"}, {"Name": (this.commonService.Res$.getValue())._category, "Value": "category"},{"Name": (this.commonService.Res$.getValue())._link, "Value": "link"}];

      if (typeof this.controlOptions.items != "undefined") {
        //debugger;
        if(this.field.Values[0].Value.startsWith("product:")) {
          this.selectedTagType = "product";
        } else if(this.field.Values[0].Value.startsWith("category:")) {
          this.selectedTagType = "category";
        } else if(this.field.Values[0].Value.startsWith("link:")) {
          this.selectedTagType = "link";
        }
        let _selectedValue:any;
        if (this.field.Values[0].Value.trim() != "") {
          _selectedValue = _(this.controlOptions.items).keyBy('value')
              .at(this.field.Values[0].Value.replace("product:", "").replace("category:", "").replace("link:", "").split(",")).value();
          _selectedValue = _selectedValue.filter(function(n){ return n != undefined });
          this.controlOptions.selectedValue = _selectedValue;
        }
        else {
  ...
  SearchTags = (text:string) => {
    if (this.field.Type == "tags-product" || (this.field.Type == "product-or-category" && this.selectedTagType == "product"))
      return this.sectionService.SearchProducts(text);
    else if (this.field.Type == "tags-category" || (this.field.Type == "product-or-category" && this.selectedTagType == "category"))
      return this.sectionService.SearchCategories(text);
    else if (this.field.Type == "tags-link" || (this.field.Type == "product-or-category" && this.selectedTagType == "link"))
      return this.sectionService.SearchLinks(text);
  }
