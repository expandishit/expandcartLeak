...
_fnCompatOpts(defaults);_fnCompatCols(defaults.column);_fnCamelToHungarian(defaults,defaults,!0);_fnCamelToHungarian(defaults.column,defaults.column,!0);_fnCamelToHungarian(defaults,$.extend(oInit,$this.data()),!0);var allSettings=DataTable.settings;for(i=0,iLen=allSettings.length;i<iLen;i++)
...
{_fnCompatCols(oOptions);_fnCamelToHungarian(DataTable.defaults.column,oOptions,!0);if(oOptions.mDataProp!==undefined&&!oOptions.mData)
...
{oCol=oSettings.aoColumns[i];create=nTrIn?!1:!0;nTd=create?document.createElement(oCol.sCellType):anTds[i];nTd._DT_CellIndex={row:iRow,column:i};cells.push(nTd);if(create||((!nTrIn||oCol.mRender||oCol.mData!==i)&&(!$.isPlainObject(oCol.mData)||oCol.mData._!==i+'.display'))){nTd.innerHTML=_fnGetCellData(oSettings,iRow,i,'display')}
if(oCol.sClass)
...
{var
start=parseInt(settings._iDisplayStart),len=parseInt(settings._iDisplayLength),records=settings.fnRecordsDisplay();if(records===0||len===-1)
{start=0}
