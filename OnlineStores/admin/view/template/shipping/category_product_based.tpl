...
<style type="text/css">
	...
	input.form-control, select.form-control { z-index: 0 !important; }
	hr { margin: 10px 0; }
...
<div id="content">
	...
	<?php if (isset($this->session->data['error'])) { ?>
        <script>
            var notificationString = '<?php echo $this->session->data['error']; ?>';
            var notificationType = 'warning';
        </script>
	...
	<div class="box">
        <div class="heading">
            ...
            <div class="buttons">
                <a onclick="saveSettings($(this));" class="button btn btn-primary"><?php echo $button_save; ?></a>
                <a href="<?php echo $exit; ?>" class="button btn btn-primary"><?php echo $button_cancel; ?></a>
            </div>
        </div>
				...
				<?php } elseif ($setting['type'] == 'typeahead') { ?>
						...
						<div class="setting typeahead-block">
							<?php if (empty($setting['typeahead'])) { ?>
								...
								<select class="nosave form-control" style="margin: 0 0 5px 0" data-help='<?php echo $help_autocomplete_from; ?>'>
