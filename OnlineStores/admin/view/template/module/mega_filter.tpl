...
////////////////////////////////////////////////////////////////////////////////
	...
	jQuery('.scrollbox div i').on('click', function() {
		...
		$module.find('input[name="category-name"]:not([autocomplete])').each(function(){
			...
			jQuery(this).attr('autocomplete','1').autocomplete({
				...
				select: function(event, ui) {
					...
					$c.append('<div>' + ui.item.label + '<i class="fa fa-times"></i><input type="hidden" name="<?php echo $_name; ?>_module[category_id][]" value="' + ui.item.value + '" /></div>');
		...
		$module.find('input[name="category-hide-name"]:not([autocomplete])').each(function(){
			...
			jQuery(this).attr('autocomplete','1').autocomplete({
				...
				select: function(event, ui) {
					...
					$c.append('<div>' + ui.item.label + '<i class="fa fa-times"></i><input type="hidden" name="<?php echo $_name; ?>_module[hide_category_id][]" value="' + ui.item.value + '" /></div>');
	...
	createScrollbox: function( id, type, name, items, key, values ) {
		...
		for( i in items ) {
			...
			switch( type ) {
				...
				case 'delete' : {
					$div.append( items[i] );
					$div.append( '<i class="fa fa-times"></i>' );
