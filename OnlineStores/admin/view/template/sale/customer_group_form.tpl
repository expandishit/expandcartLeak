      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <table class="form">
          ...
          <tr>
            ...
            <tr>
                <td>{{ lang('enable_email_activation') }}</td>
                <td>
                    <select name="email_activation">
                        <option value="1" <?= $email_activation == 1 ? 'selected' : '';?> >{{ lang('text_enabled') }}</option>
                        <option value="0" <?= $email_activation == 0 ? 'selected' : '';?> >{{ lang('text_disabled') }}</option>
                    </select>
                </td>
            </tr>
            <tr>
                <td>{{ lang('enable_sms_activation') }}</td>
                <td>
                    <?php if ($smsModule['status'] == 1) { ?>
                    <select name="sms_activation">
                        <option value="1" <?= $sms_activation == 1 ? 'selected' : '';?> >{{ lang('text_enabled') }}</option>
                        <option value="0" <?= $sms_activation == 0 ? 'selected' : '';?> >{{ lang('text_disabled') }}</option>
                    </select>
                    <?php } else { ?>

                    {{ lang('sms_module_is_not_installed') }}

                    <?php } ?>
                </td>
            </tr>
            <tr>
                <td style="color: tomato;font-weight: bold;" colspan="2">{{ lang('enable_sms_activation_note') }}</td>
            </tr>
          <tr>
