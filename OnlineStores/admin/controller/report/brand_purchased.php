<?php
class ControllerReportBrandPurchased extends Controller {
	...
	public function index() {   
        ...
        $this->data['filter_date_end'] = date('y/m/d');
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('report/brand_purchased', '', 'SSL'),
      		'separator' => ' :: '
   		);
        ...
        $this->data['action'] = $this->url->link('report/brand_purchased/ajaxResponse', $comming_from, 'SSL');
		...
		$this->data['export_link'] = $this->url->link('tool/w_export_tool/exportBrandPurchasedReport', '', 'SSL');
		
		$this->template = 'report/brand_purchased.expand';
		$this->children = array(
			'common/header',
			'common/footer'
		);
    ...
    public function ajaxResponse()
    {
        ...
        $product_total = $this->model_report_brand->getTotalPurchased($data);

        $totalData = $product_total;
        $totalFiltered = $totalData;


        $results = $this->model_report_brand->getPurchasedDataTable($data, $requestData, $columns);



        $alt = array();
        foreach ($results['data'] as $result) {
            $alt[] = [$result['name'],$result['current_quantity'], $result['quantity'],
