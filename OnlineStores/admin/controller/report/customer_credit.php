	...
	public function index() {     
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('report/customer_credit', '', 'SSL'),
      		'separator' => ' :: '
   		);		
        ...
        $this->data['action'] = $this->url->link('report/customer_credit/ajaxResponse', $comming_from, 'SSL');
        ...
        $this->data['export_link'] = $this->url->link('tool/w_export_tool/exportCustomerCreditReport', '', 'SSL');
				 
		$this->template = 'report/customer_credit.expand';
		$this->children = array(
			'common/header',
			'common/footer'
		);
...
//        }
        ...
        $order_total = $this->model_report_customer->getTotalCredit($data);

        $totalData = $order_total;
        $totalFiltered = $totalData;


        $results = $this->model_report_customer->getCreditDataTable($data, $requestData, $columns);


        $alt = array();
        foreach ($results['data'] as $result) {
            $alt[] = [$result['customer'],
                $result['email'], $result['customer_group'], $result['status'], $result['total'],['text' => $this->language->get('text_edit'),
...
//        die();
        ...
        $json_data = array(
            ...
            "iTotalDisplayRecords" => intval($totalFiltered), // total number of records
