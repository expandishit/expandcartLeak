...
class ControllerTotalRedeemPoint extends Controller {
	...
	public function index() {
		...
		$this->data['action'] = $this->url->link('total/redeem_point', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['cancel'] = $this->url->link('extension/total', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->post['redeem_point_status'])) {
			$this->data['redeem_point_status'] = $this->request->post['redeem_point_status'];
		} else {
			$this->data['redeem_point_status'] = $this->config->get('redeem_point_status');
		}

		if (isset($this->request->post['redeem_point_sort_order'])) {
			$this->data['redeem_point_sort_order'] = $this->request->post['redeem_point_sort_order'];
		} else {
			$this->data['redeem_point_sort_order'] = $this->config->get('redeem_point_sort_order');
		}

		$this->template = 'promotions/reward_points/total/redeem_point.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
