...
class ControllerWkposCredit extends Controller {
  ...
  public function index() {
    $this->data = $this->load->language('wkpos/credit');
    ...
    $this->data['balance'] = $this->model_wkpos_credit->getCreditTotal($this->request->get['customer_id']);
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('wkpos/credit', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', true);

		$this->data['pagination'] = $pagination->render();

		$this->data['results'] = sprintf($this->language->get('text_pagination'), ($reward_total) ? (($page - 10) * 10) + 1 : 0, ((($page - 10) * 10) > ($reward_total - 10)) ? $reward_total : ((($page - 10) * 10) + 10), $reward_total, ceil($reward_total / 10));

    $this->children = array(
           'common/header',
           'common/footer',
        );
    $this->template = 'wkpos/credit.tpl';
    $this->response->setOutput($this->render(TRUE));

		//$this->response->setOutput($this->load->view('wkpos/credit.tpl', $data));
  }
