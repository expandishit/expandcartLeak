...
class ControllerPaymentStripe2 extends Controller
{
    ...
    public function index()
    {
		...
		foreach ($this->model_localisation_currency->getCurrencies() as $currency) {
			$this->data['currencies'][] = $currency['code'];
        }
    ...
    private function validate()
    {
        ...
        if (!$this->request->post["{$this->paymentName}_published_key"]) {
            $this->error["{$this->paymentName}_published_key"] = $this->language->get('error_published_key');
        }
    ...
    {
        return [
            'status', 'test_mode', 'published_key', 'secret_key', 'completed_order_status_id', 'geo_zone_id',
            'completed_order_currency'
        ];
    }
}
