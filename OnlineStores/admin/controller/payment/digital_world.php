...
class ControllerPaymentDigitalWorld extends Controller
{
    ...
    private function validate()
    {
        ...
        if (!$this->request->post["{$this->paymentName}_merchant_email"]) {
            $this->error["{$this->paymentName}_merchant_email"] = $this->language->get("error_merchant_email");
        }
        ...
        if (!$this->request->post["{$this->paymentName}_site_url"]) {
            $this->error["{$this->paymentName}_site_url"] = $this->language->get("error_site_url");
        }
    ...
    {
        return [
            "status", "test_mode", "merchant_email", "secret_key", "site_url", "currency", "completed_order_status_id", 
            "geo_zone_id", "default_currency","failed_order_status_id"
        ];
    }
