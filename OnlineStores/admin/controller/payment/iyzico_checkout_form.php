...
class ControllerPaymentIyzicoCheckoutForm extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') ) {
            ...
            $this->model_setting_setting->checkIfExtensionIsExists('payment', 'iyzico_checkout_form', true);

            $this->model_setting_setting->insertUpdateSetting('iyzico_checkout_form', $this->request->post);
        ...
        if (isset($this->request->get['update_error'])) {
            $this->data['error_warning'] = $this->language->get('entry_error_version_updated');
        } else {
        ...
        $this->data['breadcrumbs'][] = array(
            'href'      => $this->url->link('payment/iyzico_checkout_form'),
            'text'      => $this->language->get('heading_title'),
            'separator' => ' :: '
        );

        $this->data['action'] = $this->url->link('payment/iyzico_checkout_form');
        ...
        $merchant_keys_name_array = array(
            ...
            'iyzico_checkout_form_test_mode',
        ...
        $this->id       = 'content';
        $this->template = 'payment/iyzico_checkout_form.expand';
    ...
    public function update() {
        ...
        if ($updated == 1) {
			...
			$this->load->controller('modification/refresh');
		    $this->session->data['success'] = $this->language->get('text_success');
            $this->response->redirect($this->url->link('extension', 'token=' . $this->session->data['token'] . '&type=payment', true));
        } else {
