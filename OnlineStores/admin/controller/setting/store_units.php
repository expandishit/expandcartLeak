<?php
class ControllerSettingStoreUnits extends Controller
{
    private $error = array();
 
    
    public function lengthDTHandler()
    {
        ...
        $return = $this->model_localisation_length_class->handler($start, $length, $lang_id, $search, $orderColumn, $orderType);
    ...
    public function lengthDTDelete()
    {
        ...
        if ( is_array($ids) )
        {
            foreach ( $ids as $length_id )
            {
                $this->model_localisation_length_class->deleteLengthClass($length_id);
            }
        }
        else
        {
            $this->model_localisation_length_class->deleteLengthClass($ids);
        }
    ...
    public function weightDTHandler()
    {
        ...
        $return = $this->model_localisation_weight_class->handler($start, $length, $lang_id, $search, $orderColumn, $orderType);
    ...
    public function weightDTDelete()
    {
        if ( $this->request->server['REQUEST_METHOD'] != 'POST' )
        {
            return;
        ...
        if ( is_array($ids) )
        {
            foreach ( $ids as $weight_id )
            {
                $this->model_localisation_weight_class->deleteWeightClass($weight_id);
            }
        }
        else
        {
            $this->model_localisation_weight_class->deleteWeightClass($ids);
        }
    ...
    {
        $this->language->load('setting/store_units');
    ...
    protected function getList()
    {
        ...
        $this->data['breadcrumbs'][] = array(
            'text'      => $this->language->get('mn_setting_menu_units'),
            'href'      => $this->url->link('settings/store_units', 'token=' . $this->session->data['token'] . $url, 'SSL'),
            'separator' => ' :: '
        );
        ...
        $this->template = 'setting/store_units.expand';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    private function getForm()
    {
        ...
        $this->template = 'localisation/length_class_form.expand';
        $this->children = array(
            'common/header',
            'common/footer'
        );
