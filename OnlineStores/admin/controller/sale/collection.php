<?php
class ControllerSaleCollection extends Controller {
	private $error = array();
    ...
    public function confirm_collection() {
        ...
        if (isset($this->request->get['order_id'])) {
            $orderData = array(
                'order_status_id' => $this->config->get('config_order_shipped_status_id'),
                'notify' => 0,
                'comment' => ""
            );

            $this->model_sale_order->addOrderHistory($this->request->get['order_id'], $orderData);
            ...
            $this->redirect($this->url->link('sale/collection', '', 'SSL'));
        }
    ...
    public function update() {
  	...
  	protected function getList() {
		if (isset($this->request->get['filter_order_id'])) {
			...
			$filter_order_status_id = $this->config->get('config_order_cod_status_id');
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/collection', '', 'SSL'),
      		'separator' => ' :: '
   		);

		$this->data['invoice'] = $this->url->link('sale/collection/invoice', '', 'SSL');
		$this->data['insert'] = $this->url->link('sale/collection/insert', '', 'SSL');
		$this->data['delete'] = $this->url->link('sale/collection/delete', '', 'SSL');

        $this->data['display_invoice'] = $this->user->hasPermission('access', 'sale/collection');
        $this->data['display_view'] = $this->user->hasPermission('access', 'sale/order');
        $this->data['display_insert'] = $this->user->hasPermission('modify', 'sale/order');
        $this->data['display_delete'] = $this->user->hasPermission('modify', 'sale/order');
    	...
    	foreach ($results as $result) {
            ...
            if ($this->user->hasPermission('access', 'sale/order')) {
                $action[] = array(
                    ...
                    'href' => $this->url->link('sale/collection/info', 'order_id=' . $result['order_id'] . $url, 'SSL')
                );
            }
            ...
            if ($this->user->hasPermission('modify', 'sale/order')) {
                if (strtotime($result['date_added']) > strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
                    $action[] = array(
                        ...
                        'href' => $this->url->link('sale/collection/update', 'order_id=' . $result['order_id'] . $url, 'SSL')
                    );
                }
            }

            if ($this->data['isCODCollector']) {
                $action[] = array(
                    'text' => $this->language->get('text_confirm_collection'),
                    'href' => $this->url->link('sale/collection/confirm_collection', 'order_id=' . $result['order_id'] . $url, 'SSL')
                );
            }
		...
		$this->data['sort_order'] = $this->url->link('sale/collection', 'sort=o.order_id' . $url, 'SSL');
		$this->data['sort_customer'] = $this->url->link('sale/collection', 'sort=customer' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('sale/collection', 'sort=status' . $url, 'SSL');
		$this->data['sort_total'] = $this->url->link('sale/collection', 'sort=o.total' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('sale/collection', 'sort=o.date_added' . $url, 'SSL');
		$this->data['sort_date_modified'] = $this->url->link('sale/collection', 'sort=o.date_modified' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('sale/collection', 'page={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();

		$this->data['filter_order_id'] = $filter_order_id == "null" ? null : $filter_order_id;
		...
		$this->template = 'sale/order_collection.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	public function getForm() {
		...
		if (!isset($this->request->get['order_id'])) {
			$this->data['action'] = $this->url->link('sale/collection/insert', '', 'SSL');
		} else {
			$this->data['action'] = $this->url->link('sale/collection/update', 'order_id=' . $this->request->get['order_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('sale/collection', '', 'SSL');
	...
	public function info() {
		...
		if ($order_info) {
            ...
            $this->data['entry_aramex'] = $this->language->get('entry_aramex');
			...
			$this->data['invoice'] = $this->url->link('sale/collection/invoice', 'order_id=' . (int)$this->request->get['order_id'], 'SSL');
            $this->data['aramex_sipment'] = $this->url->link('sale/aramex_create_shipment', 'order_id=' . (int)$this->request->get['order_id'], 'SSL');
			$this->data['cancel'] = $this->url->link('sale/collection', '', 'SSL');
				...
				foreach ($options as $option) {
					if ($option['type'] != 'file') {
						...
						$option_data[] = array(
							...
							'type'  => $option['type'],
							'href'  => $this->url->link('sale/collection/download', 'order_id=' . $this->request->get['order_id'] . '&order_option_id=' . $option['order_option_id'], 'SSL')
						);						
					}
				}
	...
	public function history() {
		...
		$pagination->limit = 10; 
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/collection/history', 'order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');
