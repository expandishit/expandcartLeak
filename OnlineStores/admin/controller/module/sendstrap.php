<?php
// -----------------------------
// Integration With sendstrap
// -----------------------------
class ControllerModuleSendStrap extends Controller {
	private $error = array();
	private $_name = 'sendstrap';

	public function index() {
        $this->load->model('marketplace/common');
        $market_appservice_status = $this->model_marketplace_common->hasApp('sendstrap');
		...
		if (isset($this->request->post['sendstrap_id'])) {
            $this->data['sendstrap_id'] = $this->request->post['sendstrap_id'];
        } else {
            $this->data['sendstrap_id'] = $this->config->get('sendstrap_id');
		}
		
        if (isset($this->request->post['sendstrap_token'])) {
            $this->data['sendstrap_token'] = $this->request->post['sendstrap_token'];
        } else {
            $this->data['sendstrap_token'] = $this->config->get('sendstrap_token');
        }
