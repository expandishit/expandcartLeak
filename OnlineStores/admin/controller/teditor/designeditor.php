<?php
class ControllerTeditorDesigneditor extends Controller {
    ...
    public function index() {
        ...
        if (!in_array(STORECODE,$white_list)){
            $this->response->redirect($this->url->link('teditor/teditor', '', true));
        }*/
        ...
        /**
         * HANDLE INTERCOME SETTINGS
         */
        $data['intercomEnabled'] = false;        
        if ($this->user->hasPermission('access', 'billingaccount/plans') && ($this->session->data['user_id'] != 999999999) && ENABLE_MARKETING == '1'){
            $this->load->model('billingaccount/common');
            $data['billingDetails'] = $this->model_billingaccount_common->getBillingDetails();
            $data['intercomSettings'] = [
                "app_id" => "uvmgsp7k",
                "user_id"=> STORECODE,
                "language_override"=> $data['langcode'],
                "name"=> $data['billingDetails']['name'],
                "email"=> $data['billingDetails']['email']
            ];
            $data['intercomEnabled'] = true;        
        }
        // End 
        $this->data = $data;
        $this->data['title'] = "Template Editor";
        $this->template = 'designeditor/index.html';

        /***************** Start ExpandCartTracking #347718  ****************/
        $extra_routes = [
            'Editor'            => "Designed Editor",
        ...
        $this->trackOperations('Visit Route',$extra_routes);
    ...
    public function getTemplateStructure() {       
        if(IS_NEXTGEN_FRONT) {
            ...
            foreach($web_pages as &$page){
                ...
                $page['prview'] = $url->frontUrl('information/information', 'information_id='.$page['information_id'], true, false);
            }
    ...
    public function Publish() {
        if(IS_NEXTGEN_FRONT && isset($_POST)) {
            ...
            $this->trackOperations('Publish Template',$meta_data);
    ...
    public function insertSection() {
        if(IS_NEXTGEN_FRONT
            ...
            // $section_contents = $this->render_section($section_data);
            // $json['sectionHTML'] = $section_contents['content'];
            // $json['sectionScripts'] = $section_contents['scripts'];
            ...
            $this->trackOperations('Edit Template',$meta_data);
    ...
    public function SaveFieldsVal() {
        if(IS_NEXTGEN_FRONT && isset($_POST)) {
            ...
            foreach($_POST as $key => $value) {
                ...
                //$json[$key] = $value;
                $this->model_teditor_teditor->saveFieldValById($key, $value);
    ...
    public function getLinks() {
        ...
        designeditorLog([
            'note' => '$data_type = ' . $data_type . ' - designeditor controller'
        ]);

        if (strpos($data_type, 'select') !== false){
            designeditorLog([
                'note' => 'entered if - getLinks function - designeditor controller'
            ]);
            $json = $this->model_teditor_link_manager->getLinks($this->request->get['search']);
        }else{
            designeditorLog([
                'note' => 'entered else - getLinks function - designeditor controller'
            ]);
            $json['Links'] = $this->model_teditor_link_manager->getLinks($this->request->get['search'], $data_type);
        }
    ...
    private function getFieldVals($fieldId){
        ...
        foreach($fieldVals as &$fieldVal){
            ...
            if($fieldVal['Type'] == "tags-product"){
                $fieldVal['tags-product'] = $this->model_teditor_teditor->getProducts($fieldVal['Value']);
            }
            else if($fieldVal['Type'] == "tags-category"){
                $fieldVal['tags-category'] = $this->model_teditor_teditor->getCategories($fieldVal['Value']);
            }
            else if($fieldVal['Type'] == "tags-brand"){
                $fieldVal['tags-brand'] = $this->model_teditor_teditor->getBrands($fieldVal['Value']);
            }
    ...
    protected function render_section($section_data){
        ...
        $sectionHTML = $this->render_section_ecwig();

        $sectionScripts = null;
        if(file_exists(DIR_CUSTOM_TEMPLATE . $this->config->get('config_template') . '/template/section/' . $section_data['region_codename'] . '/' . $section_data['section_codename'] . '.js')){
            $sectionScripts = file_get_contents(DIR_CUSTOM_TEMPLATE . $this->config->get('config_template') . '/template/section/' . $section_data['region_codename'] . '/' . $section_data['section_codename'] . '.js');
        }
        else if(file_exists(DIR_ONLINESTORES . FRONT_FOLDER_NAME . '/view/theme/' . $this->config->get('config_template') . '/template/section/' . $section_data['region_codename'] . '/' . $section_data['section_codename'] . '.js')) {
            $sectionScripts = file_get_contents(DIR_ONLINESTORES . FRONT_FOLDER_NAME . '/view/theme/' . $this->config->get('config_template') . '/template/section/' . $section_data['region_codename'] . '/' . $section_data['section_codename'] . '.js');
        }

        return ['content' => $sectionHTML, 'scripts' => $sectionScripts];
    }
    ...
    private function trackOperations($event_name,$meta_data=[]){
