...
class ControllerLocalisationWeightClass extends Controller {
	...
	protected function getForm() {
		...
		$this->data['cancel'] = $this->url->link('setting/store_units', '', 'SSL');
		...
		$this->template = 'localisation/weight_class_form.expand';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm()
	{
		...
		foreach ( $this->request->post['weight_class_description'] as $language_id => $value )
		{
			...
			{
				$this->error['weight_title_' . $language_id] = $this->language->get('error_title');
			}

			if ( !$value['unit'] || (utf8_strlen($value['unit']) > 4) )
			{
				$this->error['weight_unit_' . $language_id] = $this->language->get('error_unit');
			}
		}
