...
class ControllerLocalisationLengthClass extends Controller {
	...
	protected function getList() {
		...
		$this->template = 'localisation/length_class_list.expand';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		$this->data['cancel'] = $this->url->link('setting/store_units', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	private function validateForm()
	{
		...
		foreach ( $this->request->post['length_class_description'] as $language_id => $value )
		{
			...
			{
				$this->error['length_title_' . $language_id] = $this->language->get('error_title');
			}

			if ( ! $value['unit'] || (utf8_strlen($value['unit']) > 4) )
			{
				$this->error['length_unit_' . $language_id] = $this->language->get('error_unit');
			}
		}
