...
class ModelCatalogDownload extends Model {
	...
	public function deleteDownload($download_id) {
      	if ( $this->db->query("DELETE FROM " . DB_PREFIX . "download WHERE download_id = '" . (int)$download_id . "'")
	   	     && $this->db->query("DELETE FROM " . DB_PREFIX . "download_description WHERE download_id = '" . (int)$download_id . "'")
        )
        {
            return true;
    ...
    public function dtHandler($start=0, $length=10, $search = null, $orderColumn="filename", $orderType="ASC") {
        $query = "SELECT * FROM " . DB_PREFIX . "download";
        //$query = ;
        $total = $this->db->query($query)->num_rows;
        $where = "";
        if (!empty($search)) {
            $where .= "(download.filename like '%" . $this->db->escape($search) . "%'
                    OR download.download_id like '%" . $this->db->escape($search) . "%')";
            $query .= " WHERE " . $where;
        }
