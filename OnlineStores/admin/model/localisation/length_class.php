...
class ModelLocalisationLengthClass extends Model
{
	...
	{
        $query = "SELECT * FROM " . DB_PREFIX . "length_class lc LEFT JOIN " . DB_PREFIX . "length_class_description lcd ON (lc.length_class_id = lcd.length_class_id) WHERE lcd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
        ...
        if (!empty($search)) {
            $query .= "AND (";
            $query .= "lcd.title LIKE '%" . $this->db->escape($search) . "%' )";
