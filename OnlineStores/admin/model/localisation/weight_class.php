...
class ModelLocalisationWeightClass extends Model
{
	...
	{
        $query = "SELECT * FROM " . DB_PREFIX . "weight_class wc LEFT JOIN " . DB_PREFIX . "weight_class_description wcd ON (wc.weight_class_id = wcd.weight_class_id) WHERE wcd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
        ...
        if (!empty($search)) {
            $query .= "AND (";
            $query .= "wcd.title LIKE '%" . $this->db->escape($search) . "%' )";
