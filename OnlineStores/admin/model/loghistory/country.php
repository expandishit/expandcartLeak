<?php
class ModelLoghistoryCountry extends Model {
    ...
    public function ajaxResponse($data, $request)
    {
        ...
        $sql = "SELECT lh.log_history_id,lh.action,lh.date_added,cl.name,u.email ,CONCAT(u.firstname,' ' ,u.lastname) as username  FROM `" . DB_PREFIX . "log_history` lh LEFT JOIN `" . DB_PREFIX . "user` u ON (lh.user_id = u.user_id) ";
        $sql .= " LEFT JOIN `" . DB_PREFIX . "countries_locale` cl ON (lh.reference_id = cl.country_id) ";
        $implode = array();
        ...
        $implode[] = " lh.type = 'country' ";

        $implode[] = " cl.lang_id = ". $this->config->get('config_language_id');
        ...
        if (!empty($data['search'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
            $sql .= " AND ( cl.name LIKE '" . $request['search']['value'] . "%' )";
        }
        ...
        $total = $this->getTotalcountries($data);
