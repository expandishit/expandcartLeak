<?php
class ModelLoghistoryCustomerPoint extends Model {
    ...
    public function ajaxResponse($data, $request)
    {
        ...
        foreach ($logData['data'] as $key=> $info){
            ...
            $old_points_value = isset($old_value['old_reward']) ? $old_value['old_reward'] : 0;
            $logData['data'][$key]['old_value'] = $old_points_value;
            $logData['data'][$key]['new_value'] = $new_value['points'];
        }

        $logData['total'] = $this->getTotalCustomersPoints($data);
