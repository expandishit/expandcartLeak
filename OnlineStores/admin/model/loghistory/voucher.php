<?php
class ModelLoghistoryVoucher extends Model {
    ...
    public function ajaxResponse($data, $request)
    {
        ...
        $sql = "SELECT lh.log_history_id,lh.action,lh.date_added,v.amount,v.code,u.email ,CONCAT(u.firstname,' ' ,u.lastname) as username  FROM `" . DB_PREFIX . "log_history` lh LEFT JOIN `" . DB_PREFIX . "user` u ON (lh.user_id = u.user_id) ";
        $sql .= " LEFT JOIN `" . DB_PREFIX . "voucher` v ON (lh.reference_id = v.voucher_id) ";
        $implode = array();
        ...
        if (!empty($data['search'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
            $sql .= " AND ( v.amount LIKE '" . $request['search']['value'] . "%' ";
            $sql .= " OR v.code LIKE '" . $request['search']['value'] . "%' )";
        }
        ...
        $total = $this->getTotalVouchers($data);
