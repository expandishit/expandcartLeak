...
class ModelShippingFastcoo extends Model
{
     ...
     * @var string
     */
    protected $settingsGroup = 'fastcoo';
    ...
    public function addShipmentDetails($orderId, $details, $status)
    {
        ...
        $fields[] = 'shipment_operator="fastcoo"';
    ...
    {
       $result = $this->db->query("SELECT * FROM  " . DB_PREFIX . "shipments_details where `order_id` = $orderId AND `shipment_operator` = 'fastcoo' ");
    ...
    {
        $result = $this->db->query("DELETE FROM  " . DB_PREFIX . "shipments_details where `order_id` = $orderId AND `shipment_operator` = 'fastcoo' ");
