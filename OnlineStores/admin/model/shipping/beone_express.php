...
class ModelShippingBeoneExpress extends Model
{
     ...
     * @var string
     */
    protected $settingsGroup = 'beone_express';
    ...
    public function addShipmentDetails($orderId, $details, $status)
    {
        ...
        $fields[] = 'shipment_operator="beone_express"';
    ...
    {
       $result = $this->db->query("SELECT * FROM  " . DB_PREFIX . "shipments_details where `order_id` = $orderId AND `shipment_operator` = 'beone_express' ");
    ...
    {
        $result = $this->db->query("DELETE FROM  " . DB_PREFIX . "shipments_details where `order_id` = $orderId AND `shipment_operator` = 'beone_express' ");
