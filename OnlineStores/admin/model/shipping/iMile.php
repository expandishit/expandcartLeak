...
class ModelShippingiMile extends Model
{
    ...
    public function addShipmentDetails($orderId, $details, $status)
    {
        ...
        $fields[] = 'shipment_operator="iMile"';
    ...
    {
       $result = $this->db->query("SELECT * FROM  " . DB_PREFIX . "shipments_details where `order_id` = $orderId AND `shipment_operator` = 'iMile' ");
    ...
    {
        $result = $this->db->query("DELETE FROM  " . DB_PREFIX . "shipments_details where `order_id` = $orderId AND `shipment_operator` = 'iMile' ");
