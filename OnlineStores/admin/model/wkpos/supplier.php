...
class ModelWkposSupplier extends Model {
	...
	public function getSuppliersDt($data = [], $filterData = []) {
		$query = $fields = [];
	    ...
	    $query[] = 'SELECT ' . $fields . ' FROM `' . DB_PREFIX . 'wkpos_supplier`';
	    ...
	    if (isset($filterData['search'])) {
	      ...
	      if (trim($filterData['search'][0]) == '#') {
	          $supplier_id = preg_replace('/\#/', '', $filterData['search']);
	          $query[] = "supplier_id LIKE '%{$supplier_id}%'";
	      } else {
	          $query[] = "email LIKE '%{$filterData['search']}%'";
	          $query[] = 'OR CONCAT(firstname, " ", lastname) LIKE "%' . $filterData['search'] . '%"';
	          $query[] = "OR company LIKE '%{$filterData['search']}%'";
	      }
	      $query[] = ')';
	    }
    ...
    /**
     * Assigns product to supplier
     * @param  int $product_id contains the id of product
     * @param  int $supplier   contains the quantity of product to be assigned
     * @return null             none
     */
    public function assignProduct($product_id, $supplier) {
        $pos_product = $this->db->query("SELECT * FROM " . DB_PREFIX . "wkpos_supplier_product WHERE product_id = '" . $product_id . "' AND supplier_id = '" . (int)$supplier . "'")->row;

        if (!isset($pos_product['supplier_product_id'])) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "wkpos_supplier_product SET min_quantity = 1, max_quantity = 1, product_id = '" . $product_id . "', supplier_id = '" . (int)$supplier . "', status=1");
        }
    }
