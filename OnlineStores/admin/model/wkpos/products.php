...
class ModelWkposProducts extends Model {
	...
	public function generateBarcode($product_id) {
		...
		$query = $this->db->query("SELECT sku, upc, ean, barcode FROM " . DB_PREFIX . "product WHERE product_id = " . (int)$product_id . "")->row;
	...
	public function massBarcode($selected, $all) {
		if ($all) {
			$barcodes = $this->db->query("SELECT pd.name, wb.barcode ,p.price FROM " . DB_PREFIX . "wkpos_barcode wb LEFT JOIN " .
                DB_PREFIX . "product_description pd ON (wb.product_id = pd.product_id) " .
                " LEFT JOIN  " .DB_PREFIX." product p on (wb.product_id = p.product_id) "
                ." WHERE pd.language_id = '".
                $this->config->get('config_language_id') . "'")->rows;
		} else {
			//$select = "'" . implode("','", $selected) . "'";
			$sql = "SELECT pd.name, wb.barcode ,p.price FROM " . DB_PREFIX . "wkpos_barcode wb LEFT JOIN " .
                DB_PREFIX . "product_description pd ON (wb.product_id = pd.product_id) " .
                " LEFT JOIN  " .DB_PREFIX." product p on (wb.product_id = p.product_id) "
                ." WHERE pd.language_id = '" .
                $this->config->get('config_language_id') . "' AND wb.product_id IN (" . $selected . ")";
    ...
    public function initBarcodes($product_ids= null) {
	    if ($product_ids){

            $sql = "SELECT p.product_id  FROM "
                . DB_PREFIX . "product p "
                ." left join ".DB_PREFIX."wkpos_barcode wb on wb.product_id = p.product_id  "
                ." WHERE (p.barcode != '' AND p.barcode IS NOT NULL) "
                ." AND wb.product_id IS NULL ";
            $sql .= " AND ( p.product_id in (".$product_ids.") )";

            $product_ids_rows = $this->db->query($sql)->rows;
            $product_ids=null;
            foreach ($product_ids_rows as $product_id) {
            ...
            if ($product_ids){
                $sql = "INSERT INTO " . DB_PREFIX .
                    "wkpos_barcode(product_id, barcode) SELECT product_id, barcode FROM " .DB_PREFIX.
                    "product WHERE product_id IN (".$product_ids.") ";
                $this->db->query($sql) ;
            }
        }else{
            $productBarcodes = $this->db->query("SELECT count(product_id) as brc_count FROM " . DB_PREFIX . "product WHERE barcode != '' AND barcode IS NOT NULL")->row;
            $productBarcodes = $productBarcodes['brc_count'];
            if($productBarcodes){
                $this->db->query("INSERT INTO " . DB_PREFIX . "wkpos_barcode(product_id, barcode) SELECT product_id, barcode FROM " . DB_PREFIX . "product WHERE barcode != '' AND barcode IS NOT NULL");
            }
        }
    }
}
