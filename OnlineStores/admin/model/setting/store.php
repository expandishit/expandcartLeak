...
class ModelSettingStore extends Model {
    ...
    /**
     * Get total stores histories by order status ids.
     *
     * @param array $order_status_ids
     *
     * @return array|bool
     */
    public function getTotalStoresByOrderStatusIds($order_status_ids)
    {
	    $query = [];
	    $query[] = 'SELECT COUNT(*) AS total FROM ' . DB_PREFIX . 'setting';
        $query[] = 'WHERE `key` = "config_order_status_id" AND';
	    $query[] = '`value` IN (' . implode(',', $order_status_ids) . ') AND store_id != "0"';
